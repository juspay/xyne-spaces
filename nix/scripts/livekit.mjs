...
if (!key || !secret || /^(local)?devkey$/.test(key) || /^(local)?devsecret$/.test(secret)) {
  throw new Error('Run just prepare to generate local LiveKit credentials');
}
const config = readFileSync(configPath, 'utf8')
  .replace(/\blocaldevkey\b/g, JSON.stringify(key))
  .replace(/\blocaldevsecret\b/g, JSON.stringify(secret));
