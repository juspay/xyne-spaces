...
export const DocumentOperationsProvider: React.FC<{
  ...
  const setGoToPageFn = React.useCallback((fn: ((pageIndex: number) => Promise<void>) | null) => {
    if (documentOperationsRef.current) {
      if (fn) {
        documentOperationsRef.current.goToPage = fn;
      } else {
...
export const withDocumentOperations = <P extends object>(
  ...
  const WrappedComponent = forwardRef<DocumentOperations, P>((props, ref) => {
  ...
  WrappedComponent.displayName = `withDocumentOperations(${Component.displayName ?? Component.name ?? 'Component'})`;
