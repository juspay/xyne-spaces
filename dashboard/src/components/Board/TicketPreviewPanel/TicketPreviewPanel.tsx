...
export const TicketPreviewPanel = ({
  ...
  return (
    <div className='w-[50%] flex flex-col bg-white rounded-[12px] shadow-lg m-4 z-10 border border-[#e4e6e7]'>
      <div className='flex items-center justify-between px-[16px] py-[12px]'>
        <h2 className='text-[14px] font-medium text-[#181b1d]'>Ticket Preview</h2>
        <Button
          onClick={onClose}
          variant='ghost'
          size='iconSm'
          className='w-[24px] h-[24px] border border-[#e4e6e7] rounded-[4px] hover:bg-[#f5f5f5]'
      ...
      {/* Bottom Navigation */}
      <div className='flex items-center justify-center gap-[6px] px-[20px] py-[16px] bg-white flex-shrink-0 rounded-[12px]'>
        <div className='inline-flex rounded-[10px] bg-[#f1f2f3] p-[4px] gap-[6px]'>
          <Button
            ...
            className={`px-[12px] py-[8px] text-[13px] font-medium rounded-[8px] transition-colors tracking-[-0.2px] h-auto ${
              previewMode === 'ticket'
                ? 'bg-white text-[#181b1d] border border-[#e4e6e7] hover:bg-white'
                : 'text-[#505b62] hover:text-[#181b1d] hover:bg-transparent'
            }`}
            data-track-category={trackCategory}
