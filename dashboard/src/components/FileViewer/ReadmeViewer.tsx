...
export const ReadmeViewer: React.FC<BaseViewerProps> = memo(({ source }) => {
  ...
  const markdownComponents: Components = useMemo(
    () => ({
      h1: ({ children }) => (
        <h1 className='text-4xl font-bold text-gray-900 dark:text-gray-100 mb-6 pb-2 border-b border-gray-200 dark:border-gray-700'>
          {children}
        </h1>
      ),
      h2: ({ children }) => (
        <h2 className='text-3xl font-semibold text-gray-900 dark:text-gray-100 mb-5 mt-8 pb-2 border-b border-gray-200 dark:border-gray-700'>
          {children}
        </h2>
      ),
      p: ({ children }) => (
        <p className='text-gray-800 dark:text-gray-200 leading-relaxed mb-4'>{children}</p>
      ),
      a: ({ href, children }) => (
        <a
          href={href}
  ...
  return (
    <div className='relative min-h-full bg-white dark:bg-[#1E1E1E] font-sans pt-[65px]'>
