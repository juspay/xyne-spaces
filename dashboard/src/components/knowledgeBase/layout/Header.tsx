...
import { getFolderCountersByFolderId } from '../../../services/Knowledge/collectionService';
...
/**
 * Header component
 * Shows view toggle, search, and upload button
 */
export const Header: React.FC<HeaderProps> = ({
  ...
  const { failedItems, hydrateAncestors } = useCollectionTree();
  ...
  const handleAskAIClick = async () => {
    ...
    const effectiveFolderId = folderId && folderId !== '_' ? folderId : collectionId;
    try {
      const res = await getFolderCountersByFolderId(collectionId, effectiveFolderId);
      const pending = res.counter?.pendingFiles ?? 0;
      const processing = res.counter?.processingFiles ?? 0;
      const total = res.counter?.totalFiles ?? 0;
      const hasProcessing = processing > 0 || pending > 0;
      if (hasProcessing) {
        setProcessingCounts({ pending, processing, total });
  ...
  // Check if user can upload (editor or owner role)
  const canUpload =
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (projectId && collectionId) {
                                ...
                                void navigate(
                                  `/knowledge-base/${projectId}/${collectionId}/${folder}/${item.id}`,
                                );
                              }
                            }}
