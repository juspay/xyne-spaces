...
const getWorkerLabel = () => {
  const data = (workerData || {}) as { workerType?: string; workerIndex?: number }
  const type = data.workerType || 'main'
  const index = typeof data.workerIndex === 'number' ? String(data.workerIndex) : '0'
...
const safeJson = (value: unknown): JsonValue => {
  ...
  if (Array.isArray(value)) return value.slice(0, 100).map(safeJson)
...
const countDirEntries = (dirPath: string): number | null => {
  try { return fs.readdirSync(dirPath).length } catch { return null }
}
...
export const recordAppSyncDiagnosticEvent = (event: string, details: DiagnosticDetails = {}) => {
  if (!enabled) return
  appendJsonLine('events.jsonl', {
    ts: new Date().toISOString(), pid: process.pid, threadId, event, details: safeJson(details),
  })
}

export const recordAppSyncSemaphoreState = (state: DiagnosticDetails = {}) => {
  if (!enabled) return
  const safeState = safeJson(state) as Record<string, JsonValue>
  atomicWriteJson('semaphore-state.json', { ts: new Date().toISOString(), pid: process.pid, threadId, ...safeState })
}

export const recordWorkerPhase = (phase: string, details: DiagnosticDetails = {}) => {
  if (!enabled) return
  const data = (workerData || {}) as { workerType?: string; workerIndex?: number }
  const payload = {
    ts: new Date().toISOString(), pid: process.pid, threadId,
    workerType: data.workerType || 'main', workerIndex: data.workerIndex ?? null,
    phase, details: safeJson(details), memoryUsage: process.memoryUsage(),
  }
...
export const startAppSyncResourceSampler = () => {
  ...
  if (!Number.isFinite(resourceSampleIntervalMs) || resourceSampleIntervalMs <= 0) return
  resourceSamplerStarted = true
  recordAppSyncDiagnosticEvent('resource_sampler_started', { diagnosticsDir, resourceSampleIntervalMs })
  const writeSample = () => {
    const payload = { ts: new Date().toISOString(), pid: process.pid, threadId, sample: collectResourceSnapshot() }
