import { PrismaClient, User, AccessType, UserPresenceStatus } from '@prisma/client';
...
export class UserService {
  ...
  async createUser(userData: GoogleUserData): Promise<User> {
    try {
      const user = await this.prisma.user.create({
        data: {
          providerUserId: userData.googleId,
          email: userData.email,
          name: userData.name,
        }
      });

      logger.info(`Created new user: ${user.email} (${user.id}) without assigning to a default group.`);
      ...
      try {
        logger.info(`Granting default access to ${DEFAULT_USER_RESOURCES.length} resources for new user ${user.email}`);
        ...
        for (const defaultResource of DEFAULT_USER_RESOURCES) {
          try {
            ...
            if (resource) {
              ...
              logger.debug(`Granted ${defaultResource.accessType} access to ${defaultResource.resourceName} for user ${user.email}`);
            } else {
              logger.warn(`Resource ${defaultResource.resourceName} not found in database for user ${user.email}`);
            }
          } catch (resourceError) {
            logger.error(`Failed to grant ${defaultResource.accessType} access to ${defaultResource.resourceName} for user ${user.email}:`, resourceError);
          }
        }
  ...
  private async ensureUserInGeneralChannel(user: User): Promise<void> {
    try {
      logger.info(`[GENERAL_CHANNEL] Checking general channel membership for user ${user.email} (${user.id})`);
      ...
      if (generalChannel) {
        ...
        if (!isAlreadyParticipant) {
          ...
          logger.info(`[GENERAL_CHANNEL] Adding user ${user.email} as MEMBER to channel ${generalChannel.id}`);
          const participant = await repositories.channelParticipants.addParticipant(
            generalChannel.id,
            user.id,
            'MEMBER'
          );
          logger.info(`[GENERAL_CHANNEL] ✅ Successfully added user ${user.email} to general channel. Participant ID: ${participant.id}`);
        } else {
          logger.info(`[GENERAL_CHANNEL] ⚠️ User ${user.email} is already a participant in general channel`);
        }
      } else {
        logger.warn(`[GENERAL_CHANNEL] ❌ General channel not found in database`);
      }
    } catch (channelError) {
      logger.error(`[GENERAL_CHANNEL] ❌ Failed to add user ${user.email} to general channel:`, channelError);
      // Don't throw - this shouldn't block authentication
    }
  }
   ...
   */
  async findOrCreateUser(googleUserData: GoogleUserData): Promise<{ user: User; isNewUser: boolean }> {
    try {
      ...
      if (user) {
        ...
        logger.info(`Linking existing email ${googleUserData.email} to Google ID ${googleUserData.googleId}`);
        user = await this.updateUser(user.id, {
  ...
  async getUserStats(userId: string) {
    try {
      ...
      const statusCounts = statusGroups.reduce((acc: Record<string, number>, group) => {
   ...
   */
  async storeRefreshToken(userId: string, _refreshToken: string | null, _expiryDate?: Date): Promise<User> {
    try {
      ...
      logger.info(`storeRefreshToken called for user: ${user.email} (${user.id}) - use UserSessionService instead`);
      return user;
    } catch (error) {
  ...
  async getUserByRefreshToken(_refreshToken: string): Promise<User | null> {
    try {
      logger.info('getUserByRefreshToken called - use UserSessionService.getSessionByRefreshToken() instead');
      return null;
    } catch (error) {
  ...
  async invalidateRefreshToken(userId: string): Promise<void> {
    try {
      logger.info(`invalidateRefreshToken called for user ID: ${userId} - use UserSessionService.revokeAllUserSessions() instead`);
    } catch (error) {
