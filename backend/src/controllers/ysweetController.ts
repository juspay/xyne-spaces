...
export class YSweetController {
  ...
  async duplicateCanvas(req: Request, res: Response): Promise<void> {
    try {
      const { sourceCanvasId } = req.body;

      if (!sourceCanvasId || typeof sourceCanvasId !== 'string') {
        res.status(400).json({
          error: 'Invalid request',
          message: 'sourceCanvasId is required and must be a string',
        });
        return;
      }
      ...
      let newCanvasId: string;
      try {
        ({ id: newCanvasId } = await canvasAuthService.duplicateCanvas(sourceCanvasId, userId));
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : '';
        if (errorMessage.includes('permission to create canvas')) {
        ...
        if (errorMessage === 'Canvas not found') {
          res.status(404).json({ error: 'Not found', message: 'Canvas not found' });
          return;
        }
        if (errorMessage === 'Failed to copy canvas content') {
          res.status(502).json({
            error: 'Bad Gateway',
            message: 'Failed to copy canvas content. Please try again.',
          });
          return;
        }
        throw error;
      ...
      res.json({ id: newCanvasId });
    } catch (error) {
      logger.error('Error in duplicateCanvas:', error);
      res.status(500).json({
