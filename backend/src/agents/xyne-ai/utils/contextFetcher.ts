    ...
    if (canvas.isCollaborative) {
      ...
      try {
        ...
        if (blocks && blocks.length > 0) {
          ...
          contentStatus = 'EMPTY_OR_NOT_SYNCED';
          markdownContent = '⚠️ CONTENT UNAVAILABLE - Canvas is empty or not yet synced to collaborative editor.';
          logger.warn(`[ContextFetcher] Canvas ${canvasId} - No content blocks in Y-Sweet`);
        }
      } catch (ysweetError) {
        // Y-Sweet failed - fallback to DB content
        ...
        if (canvas.content && Array.isArray(canvas.content) && canvas.content.length > 0) {
          ...
          markdownContent = '⚠️ CONTENT UNAVAILABLE - Y-Sweet unavailable and no DB backup found.';
        }
      }
    } else {
