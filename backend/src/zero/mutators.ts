  ...
  return defineMutators({
    ...
    collection: {
      createCollection: defineMutator(
        ...
        async ({ tx, args: { id, projectId, name, description, vespaDocId, permissionId, timestamp } }) => {
          ...
          if (!isParticipant) {
            throw new Error('Collection creation failed: you must be a project participant');
          }

          // Check for existing non-deleted collection with same name in this project
          const existingCollection = await db.collection.findFirst({
            where: { ownerId: authData.sub, name, projectId, deletedAt: null },
            select: { id: true },
          });
          ...
          await tx.mutate.collections.insert({
            ...
            isPrivate: true,
            totalItems: 0,
            lastUpdatedByEmail: authData.email,
            lastUpdatedById: authData.sub,
            metadata: {},
            createdAt: timestamp,
            updatedAt: timestamp,
          });
      ...
      deleteCollection: defineMutator(
        ...
        async ({ tx, args: { id, timestamp } }) => {
          ...
          const items = await db.collectionItem.findMany({
            ...
            select: { id: true, vespaDocId: true },
          });
          ...
          // Queue Vespa delete jobs for items that were indexed
          for (const item of items) {
            if (item.vespaDocId) {
      ...
      createFolder: defineMutator(
        ...
        async ({ tx, args: { id, collectionId, parentId, name, path, vespaDocId, timestamp } }) => {
          ...
          await tx.mutate.collection_items.insert({
            id,
            collectionId,
      ...
      deleteItem: defineMutator(
        ...
        async ({ tx, args: { id, collectionId, timestamp } }) => {
          ...
          // Fetch item details before soft-deleting
          const item = await db.collectionItem.findUnique({
            where: { id },
            select: { vespaDocId: true, type: true },
          });

          await tx.mutate.collection_items.update({ id, deletedAt: timestamp });

          // Collect all items needing Vespa cleanup (the item itself + descendants)
          const vespaDeleteIds: { id: string; vespaDocId: string }[] = [];
          if (item?.vespaDocId) {
            vespaDeleteIds.push({ id, vespaDocId: item.vespaDocId });
          }

          // If deleting a folder, cascade soft-delete to all descendants
          if (item?.type === CollectionItemType.FOLDER) {
            const allItems = await db.collectionItem.findMany({
              where: { collectionId, deletedAt: null },
              select: { id: true, parentId: true, vespaDocId: true },
            });

            // BFS to find all descendants
            const childrenMap = new Map<string, string[]>();
            for (const ci of allItems) {
              const key = ci.parentId ?? '';
              if (!childrenMap.has(key)) childrenMap.set(key, []);
              childrenMap.get(key)!.push(ci.id);
            }
            const itemById = new Map(allItems.map(ci => [ci.id, ci]));

            const queue = [id];
            while (queue.length > 0) {
              const current = queue.shift()!;
              for (const childId of childrenMap.get(current) ?? []) {
                await tx.mutate.collection_items.update({ id: childId, deletedAt: timestamp });
                const child = itemById.get(childId);
                if (child?.vespaDocId) {
                  vespaDeleteIds.push({ id: childId, vespaDocId: child.vespaDocId });
                }
                queue.push(childId);
              }
            }
          ...
          for (const { id: docItemId } of vespaDeleteIds) {
            asyncTasks.push(async () => {
              try {
                await vespaQueue.addJob({
                  schema: 'file',
                ...
                logger.error(`[MUTATOR-DELETE-ITEM] Failed to queue Vespa delete for item ${docItemId}:`, error);
              }
            });
          }
        },
            ...
            asyncTasks.push(async () => {
              try {
                ...
                await notificationService.createNotification(userId, {
                  ...
                  actionUrl: `/knowledge-base/${collection.projectId}/${collectionId}`,
