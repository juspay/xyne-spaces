...
/**
 * Deletes a SAM transcript from Vespa.
 * Queues a delete job for async Vespa deletion.
 *
 * @param docId - The document ID to delete
 * @returns Promise resolving to deletion result (after queuing, not after Vespa delete)
 */
export async function deleteSamTranscriptFromVespa(
  docId: string
): Promise<SamTranscriptInsertResult> {
  try {
    logger.info('[SamTranscriptsHandler] Starting Vespa delete job queue', { docId });
    ...
    logger.info('[SamTranscriptsHandler] Successfully queued SAM transcript deletion for Vespa', {
      docId,
    });
    ...
    logger.error('[SamTranscriptsHandler] Failed to queue SAM transcript deletion for Vespa', {
      docId,
      error: errorMessage,
    });
