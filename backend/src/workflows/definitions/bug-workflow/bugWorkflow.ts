...
// Verify build in existing cloned repository (no cloning)
// async function verifyBuildInPlace(
//   repoPath: string,
...
//     logger.info(`${repositoryName}_verify_build_in_place_1_mock`, {
//       repositoryName,
//       message: 'Mock build enabled',
//     });
...
//     logger.info(`${repositoryName}_verify_build_in_place_1_mock_success`, { repositoryName });
//     return {
//       success: true,
//       exitCode: 0,
...
//   try {
//     logger.info(`🔨 [BUILD-VERIFY] Running cabal build in ${repoPath}...`);
//     logger.info(`${repositoryName}_verify_build_in_place_1_start`, { repositoryName, repoPath });
...
//           'bash',
//           '-c',
//           'cabal update && cabal build -v0 --ghc-options="-w"',
//         ],
//         {
//           cwd: repoPath,
//           stdio: ['inherit', 'pipe', 'pipe'],
...
//         const output = data.toString();
//         buildErrors += output;
//         // Log all stderr output as it may contain errors
//         const lines = output.split('\n');
//         lines.forEach((line: string) => {
//           if (line.trim()) logger.info(`[BUILD ${repositoryName}] ${line}`);
//         });
...
//         logger.info(
//           `🔍 [BUILD-DEBUG] Process closed with code: ${code}, exitCode set to: ${exitCode}`
//         );
//         logger.info(`${repositoryName}_verify_build_in_place_1_process_closed`, {
//           repositoryName,
//           code,
//           exitCode,
//         });
//         if (code === 0) {
//           logger.info(
//             `✅ [BUILD-VERIFY] Build successful for ${repositoryName} (exit code: ${code})`
//           );
//           logger.info(`${repositoryName}_verify_build_in_place_1_success`, {
//             repositoryName,
//             exitCode: code,
//           });
//           resolve();
//         } else {
//           logger.error(
//             `❌ [BUILD-VERIFY] Build failed for ${repositoryName} (exit code: ${code})`
//           );
//           logger.error(`${repositoryName}_verify_build_in_place_1_failed`, {
//             repositoryName,
//             exitCode: code,
//             buildErrors,
//             buildOutput,
//           });
//           const errorDetails = buildErrors || buildOutput || 'Build failed';
...
//       setTimeout(() => {
//         buildProcess.kill();
//         reject(new Error('Build timeout after 2.5 hours'));
//       }, 9000000); // 2.5 hour timeout
//     });
...
//       output: buildOutput,
//     };
//     logger.info(`${repositoryName}_verify_build_in_place_1_output`, result);
//     return result;
//   } catch (error) {
//     const errorMessage = error instanceof Error ? error.message : String(error);
//     logger.error(`❌ [BUILD-VERIFY] Build failed for ${repositoryName}:`, errorMessage);
//     logger.error(`${repositoryName}_verify_build_in_place_1_error`, {
//       repositoryName,
//       error: errorMessage,
...
// Named checkpoint function for build verification - follows same pattern as other checkpoints
...
//   buildOutput?: string;
// }> => {
//   logger.info(`🔨 [BUILD-CHECK] Verifying build for ${targetRepository}...`)
//   logger.info(`${targetRepository}_verify_build_1_input`, { targetRepository, clonedRepoPath });

//   if (!clonedRepoPath) {
//     const error = new Error(`Cloned repository path not found for ${targetRepository}`);
//     logger.error(`${targetRepository}_verify_build_1_error`, { targetRepository, message: 'Cloned repository path not found' });
//     throw error;
//   }

//   const buildResult = await verifyBuildInPlace(
//     clonedRepoPath,
//     targetRepository
//   )

//   if (!buildResult.success) {
//     logger.error(`❌ [BUILD-CHECK] Build failed for ${targetRepository}`)
//     const cleanError = stripAnsiCodes(buildResult.error || 'Build verification failed')
//     logger.error(`${targetRepository}_verify_build_1_build_failed`, { targetRepository, error: cleanError });
//     throw new Error(`Build failed: ${cleanError}`)
//   }

//   logger.info(`✅ [BUILD-CHECK] Build successful for ${targetRepository}`)
//   logger.info(`${targetRepository}_verify_build_1_success`, { targetRepository, exitCode: buildResult.exitCode });

//   // Strip ANSI codes from output while keeping all text
//   const result = {
//     repository: targetRepository,
//     buildSuccess: buildResult.success,
//     exitCode: buildResult.exitCode,
//     buildOutput: stripAnsiCodes(buildResult.output || '')
//   };
//   logger.info(`${targetRepository}_verify_build_1_output`, result);
//   return result;
// }

// Create xyne-cli agentic checkpoint configuration
