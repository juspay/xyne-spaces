...
export class TagService {
  ...
  async setManualTags(
    ...
    const current = await tagRepository.findActiveTags(sourceId, sourceType, tagCategory);
    ...
    for (const row of toRemove) {
      await tagRepository.softDeleteTagRow(row.id, userId);
    }
    ...
    const updated = await tagRepository.findActiveTags(sourceId, sourceType, tagCategory);
