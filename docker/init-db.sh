...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "xyne_dev_db" <<-EOSQL
    ...
    GRANT USAGE, CREATE ON SCHEMA encryption TO ${ENC_USER};
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA encryption TO ${ENC_USER};
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA encryption TO ${ENC_USER};
    ALTER DEFAULT PRIVILEGES IN SCHEMA encryption GRANT ALL PRIVILEGES ON TABLES TO ${ENC_USER};
    ALTER DEFAULT PRIVILEGES IN SCHEMA encryption GRANT ALL PRIVILEGES ON SEQUENCES TO ${ENC_USER};
EOSQL
