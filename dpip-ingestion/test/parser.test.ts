...
describe('parseDpipPayload', () => {
  it('canonicalizes party identifiers without changing other text fields', () => {
    ...
    screenings.data[1] = ['2026-07-22', 'PNB', 'NO_MATCH', 2];
  ...
  it('parses all six valid tables and converts integers to bigint', () => {
    const result = parseDpipPayload(validPayload());
  ...
  it('maps reordered headers by name', () => {
    ...
    screenings.data = [
      ['count', 'screening_status', 'party_id', 'screening_date'],
      ['2', 'REVIEW', 'party-1', '2026-07-22'],
    ];
  ...
  it('rejects a missing table', () => {
    ...
    assert.throws(
      ...
      /Missing tables: party_identifiers/,
    );
  });
