...
# Find the target report directory (supports both pnpm run test and pnpm run test:runner)
...
if [ -z "$LATEST_BUILD_DIR" ] || [ ! -d "$LATEST_BUILD_DIR" ]; then
  ...
  if ! pnpm run test --prefix "$REPO_ROOT"; then
    echo ""
    echo -e "${RED}❌ Automation tests failed. Not pushing reports.${NC}"
    echo -e "${RED}   Fix the failures and run 'pnpm run test:push' again.${NC}"
    exit 1
  fi
