...
CURRENT_BRANCH_HASH=$(git ls-files -s tools/xyne-automation/tests/ | awk '{print $2, $4}' | sort | sha256sum | awk '{print $1}')

# Calculate hash of xyne-automation folder in main branch
git fetch origin main --depth=1 2>/dev/null || true
MAIN_BRANCH_HASH=$(git ls-tree -r origin/main tools/xyne-automation/tests/ | awk '{print $3, $4}' | sort | sha256sum | awk '{print $1}')
