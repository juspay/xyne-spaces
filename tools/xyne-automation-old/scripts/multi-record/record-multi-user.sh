...
if [ -n "$REQUESTED_USERS" ]; then
    if [ "$REQUESTED_USERS" -gt "$MAX_BROWSERS" ] 2>/dev/null; then
        ...
        if [ "$CONFIRM" != "y" ] && [ "$CONFIRM" != "Y" ]; then
            echo ""
            echo -e "${CYAN}Usage: pnpm run record:multi -- --users ${MAX_BROWSERS}${NC} (max available)"
            exit 0
        fi
        echo ""
...
pnpm exec ts-node -r tsconfig-paths/register "$SCRIPT_DIR/multi-user-recorder.ts" "${PASSTHROUGH_ARGS[@]}"
