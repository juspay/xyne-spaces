...
if [ ${#SPEC_FILES[@]} -eq 0 ]; then
    ...
    echo ""
    echo "Usage:"
    echo "  pnpm run codegen -- convert <spec-files...>"
    echo ""
    echo "Flags:"
    ...
    if [ -d "$DASHBOARD_SRC" ]; then
        for spec in "${RESOLVED_SPECS[@]}"; do
            ...
            if [ $TESTID_EXIT -ne 0 ]; then
                ...
                echo -e "  ${GREEN}pnpm run codegen -- convert --skip-testids $SPEC_ARGS${NC}"
                echo ""
...
if [ "$SKIP_ANALYSIS" = true ]; then
    ...
    for spec in "${RESOLVED_SPECS[@]}"; do
        ...
        if [ $ANALYSIS_EXIT -ne 0 ]; then
            ...
            echo -e "  ${GREEN}pnpm run codegen -- convert --skip-testids --skip-analysis --retry-folder 06_canvas $SPEC_ARGS${NC}"
            echo ""
            rm -f "$ANALYSIS_OUTPUT"
            echo -e "${RED}Stopping pipeline.${NC}"
            exit 1
        ...
        if [ -f "$ANALYSIS_OUTPUT" ] && [ -s "$ANALYSIS_OUTPUT" ]; then
            ...
            echo -e "  ${GREEN}pnpm run codegen -- convert --skip-testids --skip-analysis --retry-folder <folder> $SPEC_ARGS${NC}"
            exit 0
        fi
...
if [ "$SKIP_CONVERT" = true ]; then
    ...
    for spec in "${RESOLVED_SPECS[@]}"; do
        ...
        if [ $CONVERT_EXIT -ne 0 ]; then
            ...
            if [ -n "$RETRY_FOLDER" ]; then
                echo -e "  ${GREEN}pnpm run codegen -- convert --skip-testids --skip-analysis --retry-folder $RETRY_FOLDER $SPEC_ARGS${NC}"
            else
                echo -e "  ${GREEN}pnpm run codegen -- convert --skip-testids --skip-analysis $SPEC_ARGS${NC}"
            fi
            echo ""
        fi
        echo ""
...
for spec in "${RESOLVED_SPECS[@]}"; do
    ...
    set +e
    cd "$AUTOMATION_DIR"
    echo -e "${CYAN}  Running: pnpm exec cucumber-js --dry-run $FEATURE_REL --profile e2e${NC}"
    pnpm exec cucumber-js --dry-run "$FEATURE_FILE" --profile e2e > "$DRY_RUN_REPORT" 2>&1
    DRY_RUN_EXIT=$?
    set -e
    ...
    if [ $DRY_RUN_EXIT -eq 0 ] && [ "$HAS_UNDEFINED_IN_OUTPUT" = false ]; then
        ...
        echo -e "${CYAN}  Manual dry-run:${NC} ${GREEN}cd $AUTOMATION_DIR && pnpm exec cucumber-js --dry-run $FEATURE_REL --profile e2e${NC}"
        echo ""
        ...
        if [ "$HAS_UNDEFINED" = true ]; then
            ...
            echo -e "  ${GREEN}pnpm run codegen -- convert --skip-testids --skip-analysis --retry-folder $DETECTED_FOLDER --dry-run-report $DRY_RUN_REPORT $SPEC_ARGS${NC}"
            if [ -n "$STEPS_REL" ]; then
        ...
        if [ "$HAS_AMBIGUOUS" = true ]; then
            ...
            echo -e "  ${CYAN}Then re-run:${NC} ${GREEN}cd $AUTOMATION_DIR && pnpm exec cucumber-js --dry-run $FEATURE_REL --profile e2e${NC}"
        fi

        if [ "$HAS_PARSE_ERROR" = true ]; then
            echo -e "${RED}  ❌ Syntax/Parse error${NC}"
            echo -e "  ${CYAN}Re-generate both feature + steps:${NC}"
            echo -e "  ${GREEN}pnpm run codegen -- convert --skip-testids --skip-analysis --retry-folder $DETECTED_FOLDER $SPEC_ARGS${NC}"
        fi

        if [ "$HAS_IMPORT_ERROR" = true ]; then
            echo -e "${RED}  ❌ Import/Module error${NC}"
            echo -e "  ${GREEN}pnpm install${NC}                    # Install missing packages"
            echo -e "  ${GREEN}pnpm exec tsc --noEmit${NC}               # Check TypeScript errors"
        fi
        ...
        if [ "$HAS_UNDEFINED" = true ] || [ "$HAS_IMPORT_ERROR" = true ] || [ "$HAS_PARSE_ERROR" = true ]; then
            ...
            if [ $RETRY_EXIT -eq 0 ]; then
                ...
                cd "$AUTOMATION_DIR"
                echo -e "${CYAN}  Running: pnpm exec cucumber-js --dry-run $FEATURE_REL --profile e2e${NC}"
                pnpm exec cucumber-js --dry-run "$FEATURE_FILE" --profile e2e > "$DRY_RUN_REPORT2" 2>&1
                DRY_RUN_EXIT2=$?
                set -e
                ...
                echo -e "  ${CYAN}Then dry-run:${NC} ${GREEN}cd $AUTOMATION_DIR && pnpm exec cucumber-js --dry-run $FEATURE_REL --profile e2e${NC}"
                echo ""
                echo -e "${RED}Stopping pipeline — fix the issues above before running tests.${NC}"
                exit 1
            fi
...
echo -e "${GREEN}Command: pnpm exec cucumber-js --tags \"${TAG_EXPR}\" --profile e2e${NC}"
echo ""

set +e
pnpm exec cucumber-js --tags "$TAG_EXPR" --profile e2e
TEST_EXIT=$?
set -e
...
if [ $TEST_EXIT -eq 0 ]; then
    ...
    echo ""
    echo -e "${YELLOW}💡 Recovery options:${NC}"
    echo -e "  ${CYAN}Re-run same tests:${NC} ${GREEN}pnpm exec cucumber-js --tags \"${TAG_EXPR}\" --profile e2e${NC}"
    for spec in "${RESOLVED_SPECS[@]}"; do
        ...
        if [ -n "$FF" ]; then
            ...
            echo -e "  ${CYAN}Run specific:${NC} ${GREEN}pnpm exec cucumber-js $FF_REL --profile e2e${NC}"
        fi
    done
    echo -e "  ${CYAN}Debug mode:${NC}  ${GREEN}pnpm exec cucumber-js --tags \"${TAG_EXPR}\" --profile e2e --format-options '{\"snippetInterface\":\"async-await\"}'${NC}"
fi
