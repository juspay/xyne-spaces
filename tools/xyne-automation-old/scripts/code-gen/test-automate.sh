...
if [ $# -eq 0 ]; then
    ...
    echo ""
    echo "Usage:"
    echo "  pnpm run codegen -- <file1.spec.ts> [file2.spec.ts ...]"
    echo "  pnpm run codegen -- --dry-run-report <report-file> <file.spec.ts>"
    echo "  pnpm run codegen -- --skip-folder-analysis <file.spec.ts>"
    echo "  pnpm run codegen -- --skip-scenario-analysis <file.spec.ts>"
    echo "  pnpm run codegen -- --skip-all-analysis <file.spec.ts>"
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Examples:"
    echo "  pnpm run codegen -- test-1.spec.ts"
    echo "  pnpm run codegen -- test-1.spec.ts test-2.spec.ts"
    echo "  pnpm run codegen -- ../tests/*.spec.ts"
    echo "  pnpm run codegen -- --skip-all-analysis test-1.spec.ts"
    echo "  pnpm run codegen -- --dry-run-report path/to/report.txt test-1.spec.ts"
    exit 1
fi
...
MINIMAL_STEPS_EOF
    ...
    if [ -n "$actual_feature_path" ] || [ -n "$actual_steps_path" ]; then
        ...
        if [ -n "$FEATURE_TAG" ]; then
            CMD_WITH_PREREQ="pnpm exec cucumber-js --tags \"$TAGS or @$FEATURE_TAG\" --profile e2e"
            CMD_FEATURE_ONLY="pnpm exec cucumber-js --tags \"@$FEATURE_TAG\" --profile e2e"
            CMD_FULL_E2E="pnpm exec cucumber-js --tags \"@e2e\" --profile e2e"
            ...
            CMD_WITH_PREREQ="pnpm exec cucumber-js --tags \"$TAGS\" --profile e2e $FEATURE_REL_PATH"
            CMD_FULL_E2E="pnpm exec cucumber-js --tags \"@e2e\" --profile e2e"
