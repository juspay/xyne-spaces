...
# Usage: pnpm run codegen -- <command> [options] [files...]
...
show_help() {
    ...
    echo "=========================================="
    echo ""
    echo -e "${CYAN}Usage:${NC} pnpm run codegen -- <command> [options] [files...]"
    echo ""
    ...
    echo ""
    echo "  pnpm run codegen -- convert test-1.spec.ts"
    echo "  pnpm run codegen -- convert:skip-all test-1.spec.ts"
    echo "  pnpm run codegen -- convert --dry-run-report report.txt test-1.spec.ts"
    echo "  pnpm run codegen -- convert --retry-folder 04_messages test-1.spec.ts"
    echo "  pnpm run codegen -- analyze tests/actions/test-1.spec.ts"
    echo "  pnpm run codegen -- analyze 04_messages tests/actions/test-1.spec.ts"
    echo "  pnpm run codegen -- folder-analysis tests/actions/test-1.spec.ts"
    echo "  pnpm run codegen -- scenario-analysis 04_messages tests/actions/test-1.spec.ts"
    echo "  pnpm run codegen -- add-testids tests/actions/test-1.spec.ts ../dashboard/src"
    echo "  pnpm run codegen -- cleanup"
    echo ""
    ...
    echo ""
    echo "  pnpm run codegen -- convert:skip-all tests/actions/test-1.spec.ts          # skip testid + folder + scenario analysis"
    echo "  pnpm run codegen -- convert:skip-testids tests/actions/test-1.spec.ts      # skip testid addition"
    echo ""
}
