...
if [ ${#FILTERED_PREREQ_FILES[@]} -gt 0 ]; then
    ...
    echo -e "${GREEN}pnpm exec cucumber-js$PREREQ_PATHS $FEATURE_FILE --profile e2e${NC}"
    echo ""

    # Command using tags
    echo -e "${YELLOW}# Run using tags (runs setup first, then your feature):${NC}"
    echo -e "${GREEN}pnpm exec cucumber-js --tags \"@setup or $(basename $FEATURE_FILE .feature)\" --profile e2e${NC}"
    echo ""

    # Quick run command for convenience
    echo -e "${YELLOW}# Quick run (copy-paste):${NC}"
    echo -e "${GREEN}pnpm run test:e2e -- $PREREQ_PATHS $FEATURE_FILE${NC}"
else
    echo -e "${YELLOW}# Standalone run (no prerequisites):${NC}"
    echo -e "${GREEN}pnpm exec cucumber-js $FEATURE_FILE --profile e2e${NC}"
    echo ""

    echo -e "${YELLOW}# Or with npm:${NC}"
    echo -e "${GREEN}pnpm run test:e2e -- $FEATURE_FILE${NC}"
fi
