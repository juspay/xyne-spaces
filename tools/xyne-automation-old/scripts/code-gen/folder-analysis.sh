...
# Usage: pnpm run codegen:folder-analysis -- <file.spec.ts>
...
if [ $# -eq 0 ]; then
    ...
    echo ""
    echo "Usage:"
    echo "  pnpm run codegen:folder-analysis -- <file.spec.ts>"
    echo ""
    echo "Examples:"
    echo "  pnpm run codegen:folder-analysis -- test-1.spec.ts"
    echo "  pnpm run codegen:folder-analysis -- tests/actions/test-2.spec.ts"
    exit 1
fi
...
if [ -n "$RECOMMENDED_FOLDER" ]; then
    ...
    echo -e "   ${GREEN}pnpm run codegen -- convert:skip-all --retry-folder ${RECOMMENDED_FOLDER} $INPUT_FILE${NC}"
    echo ""
