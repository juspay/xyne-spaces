...
#  One-command setup: pnpm run setup (from tools/xyne-automation/)
# ==========================================================
...
prompt_api_key() {
    ...
    if [ -z "$current_key" ] || [ "$current_key" = "your-llm-key" ]; then
        ...
        if [ -z "$current_key" ]; then
            ...
            echo -e "  ${CYAN}ℹ${NC}   Then re-run: pnpm run setup" >&2
            exit 1
        fi
    fi
...
install_deps() {
    ...
    if [ -f "package-lock.json" ] && [ -d "node_modules" ]; then
        print_info "node_modules exists, running pnpm install --frozen-lockfile..."
...
install_playwright() {
    ...
    if [ -d "node_modules/@playwright" ]; then
        ...
        pnpm install playwright --save-dev --silent
    fi
    ...
    if [ -f "$AUTOMATION_DIR/playwright.config.ts" ] || [ -f "$AUTOMATION_DIR/playwright.config.js" ]; then
        ...
        print_info "VS Code: Add ${BOLD}tools/xyne-automation/playwright.config${NC} in Playwright extension settings"
    else
        print_warning "No playwright.config found in tools/xyne-automation/"
...
print_next_steps() {
    ...
    if [ "$CHANGES_MADE" = true ]; then
        ...
        echo "     • Select: tools/xyne-automation/playwright.config"
        echo ""
        echo -e "  ${YELLOW}3. Run TestID conversion:${NC}"
        echo "     cd xyne-automation"
        echo -e "     ${BOLD}pnpm run codegen -- <spec-file.spec.ts>${NC}"
        echo ""
        echo -e "  ${YELLOW}4. Run codegen + test:${NC}"
        echo -e "     ${BOLD}pnpm run codegen-and-test -- <spec-file.spec.ts>${NC}"
        echo ""
        echo -e "  ${YELLOW}5. Cleanup generated files:${NC}"
        echo -e "     ${BOLD}pnpm run codegen-cleanup${NC}"
    else
