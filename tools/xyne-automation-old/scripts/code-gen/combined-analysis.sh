...
# Usage: pnpm run codegen -- analyze <file.spec.ts>
#        pnpm run codegen -- analyze <e2e-folder> <file.spec.ts>
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Usage:"
    echo "  pnpm run codegen -- analyze <file.spec.ts>"
    echo "  pnpm run codegen -- analyze <e2e-folder> <file.spec.ts>"
    echo ""
    echo "Arguments:"
    ...
    echo "  pnpm run codegen -- analyze 04_messages tests/actions/test-2.spec.ts"
    echo ""
    exit 1
fi
...
if [ $# -ge 2 ] && [ -d "$E2E_DIR/$1" ]; then
    ...
    if [ -d "$E2E_DIR/$1" ]; then
        ...
        echo "Usage: pnpm run codegen -- analyze [e2e-folder] <file.spec.ts>"
        exit 1
    fi
    INPUT_FILE="$1"
fi
...
case "$SCENARIO_CHOICE" in
    ...
    2)
        ...
        echo -e "  ${GREEN}pnpm run codegen -- convert:skip-all --retry-folder ${SELECTED_FOLDER} ${INPUT_FILE}${NC}"
        ;;
    3)
