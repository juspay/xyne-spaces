...
# Usage: pnpm run codegen:scenario-analysis -- <folder> <file.spec.ts>
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Usage:"
    echo "  pnpm run codegen -- scenario-analysis <file.spec.ts>"
    echo "  pnpm run codegen -- scenario-analysis <e2e-folder> <file.spec.ts>"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  pnpm run codegen -- scenario-analysis tests/actions/test-1.spec.ts"
    echo "  pnpm run codegen -- scenario-analysis 04_messages test-2.spec.ts"
    echo ""
    exit 1
fi
...
if [ $# -ge 2 ] && [ -d "$E2E_DIR/$1" ]; then
    ...
    if [ -d "$E2E_DIR/$1" ]; then
        ...
        echo "Usage: pnpm run codegen -- scenario-analysis [e2e-folder] <file.spec.ts>"
        exit 1
    fi
    INPUT_FILE="$1"
fi
...
echo -e "   ${GREEN}pnpm run codegen -- analyze $INPUT_FILE${NC}"
echo ""
echo -e "${YELLOW}2) Convert with skip analysis:${NC}"
echo -e "   ${GREEN}pnpm run codegen -- convert:skip-all --retry-folder $TARGET_FOLDER $INPUT_FILE${NC}"
echo ""
echo -e "${YELLOW}3) Full pipeline (testids + analysis + convert + dry-run + test):${NC}"
echo -e "   ${GREEN}pnpm run codegen -- convert $INPUT_FILE${NC}"
echo ""