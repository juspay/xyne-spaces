 ...
 * `20260724000000_spaces_sync_litellm_provisioning` and no Spaces sync worker
 * has populated ConnectedSurface/UserSurfaceIdentity records.
 *
 * Prerequisite: run `prisma migrate deploy` through both
 * `20260724000000_spaces_sync_litellm_provisioning` and
 * `20260805000000_add_spaces_org_member_identity` first.
 *
 ...
 * - one Claw User per Spaces org-member, with spacesOrgMemberId stamped
 ...
 *   npx tsx --env-file=.env scripts/backfill-spaces-org-identities.ts --apply --map=<spacesOrgId>:<clawOrgId>
 *
 ...
 * agent ownership from an unsafe "best guess" mapping.
 */
...
async function preflight(
  ...
  const usersByOrgMember = new Map<string, Set<string>>();
  const existingUsers = await claw.user.findMany({
    select: { id: true, orgId: true, email: true, spacesOrgMemberId: true },
  });
  ...
  const userByOrgMember = new Map<string, typeof existingUsers[number]>();
  for (const user of existingUsers) {
    userByOrgEmail.set(key(user.orgId, normalizedEmail(user.email)), user);
    if (user.spacesOrgMemberId) userByOrgMember.set(key(user.orgId, user.spacesOrgMemberId), user);
  }
  ...
  for (const sourceUser of users) {
    ...
    const byMember = userByOrgMember.get(key(targetOrgId, sourceUser.orgMemberId));
    if (byMember) candidateIds.add(byMember.id);
    if (identity) {
    ...
    const memberCandidates = usersByOrgMember.get(key(targetOrgId, sourceUser.orgMemberId)) ?? new Set<string>();
    for (const candidateId of candidateIds) memberCandidates.add(candidateId);
    usersByOrgMember.set(key(targetOrgId, sourceUser.orgMemberId), memberCandidates);
    if (candidateIds.size === 1) {
      const candidate = existingUsers.find((item) => item.id === [...candidateIds][0]);
      if (candidate?.spacesOrgMemberId && candidate.spacesOrgMemberId !== sourceUser.orgMemberId) {
        addIssue(issues, `Claw user ${candidate.id} is already linked to Spaces org member ${candidate.spacesOrgMemberId}`);
      }
    }
  }
  for (const [memberKey, candidateIds] of usersByOrgMember) {
    if (candidateIds.size > 1) {
      addIssue(issues, `One Spaces org member resolves to multiple Claw users: ${memberKey.replace("\u0000", "/")} -> ${[...candidateIds].join(", ")}`);
    }
  }
  return issues;
}
...
async function backfillOrg(
  ...
  await claw.$transaction(async (tx) => {
    ...
    for (const sourceUser of users) {
      const memberKey = key(orgId, sourceUser.orgMemberId);
      let canonicalUserId = canonicalByMember.get(memberKey);
      if (!canonicalUserId) {
        const byMember = await tx.user.findUnique({
          where: { orgId_spacesOrgMemberId: { orgId, spacesOrgMemberId: sourceUser.orgMemberId } },
          select: { id: true, email: true },
        });
        const byId = await tx.user.findUnique({ where: { id: sourceUser.id }, select: { id: true, orgId: true, email: true, spacesOrgMemberId: true } });
        const byEmail = await tx.user.findFirst({
          where: { orgId, email: { equals: sourceUser.email, mode: "insensitive" } },
          select: { id: true, email: true, spacesOrgMemberId: true },
        });
        const candidates = new Map<string, { email: string; spacesOrgMemberId?: string | null }>();
        if (byMember) candidates.set(byMember.id, byMember);
        ...
        if (candidates.size > 1) {
          throw new Error(`Spaces org member ${sourceUser.orgMemberId} resolves to multiple Claw users: ${[...candidates.keys()].join(", ")}`);
        }
        const existing = [...candidates.entries()][0];
        if (existing?.[1].spacesOrgMemberId && existing[1].spacesOrgMemberId !== sourceUser.orgMemberId) {
          throw new Error(`Claw user ${existing[0]} is already linked to Spaces org member ${existing[1].spacesOrgMemberId}`);
        }
        if (existing) {
          canonicalUserId = existing[0];
          await tx.user.update({
            where: { id: canonicalUserId },
            data: { email: sourceUser.email, name: sourceUser.name, spacesOrgMemberId: sourceUser.orgMemberId },
          });
        } else {
          ...
          await tx.user.create({
            data: {
              ...
              orgId,
              spacesOrgMemberId: sourceUser.orgMemberId,
            },
          });
        }
        canonicalByMember.set(memberKey, canonicalUserId);
      ...
      await tx.userSurfaceIdentity.upsert({
        ...
        update: { orgId, userId: canonicalUserId, status: identityStatus, linkedAt: new Date(), lastSeenAt: new Date() },
      });
    }
  });
...
async function main(): Promise<void> {
  ...
  // Applying each organization in a separate transaction is intentional: the
  // snapshot can be large. Requiring one org at a time makes the operation
  // all-or-nothing from an operator's perspective and prevents a later org
  // conflict from leaving earlier organizations partially migrated.
  if (plans.length !== 1) {
    throw new Error("Refusing --apply for multiple organizations. Re-run once per org with --org=<spacesOrgId> after reviewing each dry run.");
  }
