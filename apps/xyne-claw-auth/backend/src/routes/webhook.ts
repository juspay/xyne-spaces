...
import {
  handleSlashCommandBeforeRun,
  persistGoalStart,
  recordTurnAndDecide,
} from "../services/goalRelooper.js";
import {
  QUEUE_ENABLED,
  QUEUE_CAP,
  tryAcquireSlot,
...
import {
  getSpacesAuthForUser,
  spacesDbAvailable,
  getSpacesUserWorkspaceId,
  getWorkspaceIdForUser,
} from "../lib/spaces-db.js";
...
function parseWorkflowDefinition(definition: unknown): ChainWorkflowDefinition | null {
  ...
  const edges = raw["edges"]
    ...
    .map((e) => ({
      ...
      ...(Array.isArray(e.toolsMustInclude)
        ? { toolsMustInclude: e.toolsMustInclude.filter((t): t is string => typeof t === "string") }
        : {}),
      ...(Array.isArray(e.toolsMustExclude)
        ? { toolsMustExclude: e.toolsMustExclude.filter((t): t is string => typeof t === "string") }
        : {}),
...
async function selectNextWorkflowEdge(
  ...
  for (const edge of outgoingEdges) {
    ...
    const mode =
      edge.mode ?? (edge.toolsMustInclude?.length || edge.toolsMustExclude?.length ? "tools" : "always");
    if (mode === "always") {
...
async function pendingActionTargetValidation(
  ...
  if (conversationId) {
    try {
      ...
      if (/Spaces app API 404/i.test(msg) || (/conversation not found/i.test(msg) && /\b404\b/.test(msg))) {
        return {
          error: `conversation ${conversationId} not found — use a real Spaces conversation id, e.g. from the triggering thread`,
        };
      }
      clog.warn(
  ...
  try {
    const channel = (await spacesAppFetch("/channel/info", { channelId }, appToken)) as
      | { name?: string }
      | undefined;
    ...
    if (/Spaces app API 403/i.test(msg) || /forbidden/i.test(msg)) {
      return {
        error: `channel ${channelId} is not accessible — add the app to the channel or choose a channel it can access`,
      };
    }
    clog.warn(
...
function mergeInvocationsForCitations(...lists: unknown[]): unknown[] {
  ...
  for (const list of lists) {
    ...
    for (const inv of list) {
      const rawId =
        inv && typeof inv === "object" ? (inv as Record<string, unknown>)["toolCallId"] : undefined;
...
async function sendTwinReplyDraft(
  ...
  if (delivery.action === "ignore") {
    clog.warn(
      `[webhook/result] sendTwinReplyDraft called with action=ignore — dropping, session ${sessionId}`,
    );
  ...
  if (delivery.message && ctx.mentionedUserId) {
    try {
      ...
      clog.warn(
        `[webhook/result] Twin suffix lookup failed for user ${ctx.mentionedUserId}: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  if (effectiveDelivery.reasoning) {
    try {
      ...
      clog.warn(
        `[webhook/result] Twin citation baking failed: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  const draft = {
    ...
    ...(dest && "channelName" in dest && dest.channelName
      ? { destinationChannelName: dest.channelName }
      : {}),
    ...
    ...(effectiveDelivery.destinationReason
      ? { destinationReason: effectiveDelivery.destinationReason }
      : {}),
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      clog.error(
        `[webhook/result] Twin reply-draft create failed: ${resp.status} ${text.slice(0, 200)} — staying silent, session ${sessionId}`,
      );
      await deleteSession(sessionId);
      return;
    }
    ...
    clog.error(
      `[webhook/result] Twin reply-draft create error: ${err instanceof Error ? err.message : String(err)} — staying silent, session ${sessionId}`,
    );
    await deleteSession(sessionId);
    return;
  }
  ...
  clog.info(
    `[webhook/result] Digital Twin: posted in-thread reply draft for ${ctx.mentionedUserId} (asked by ${ctx.senderId}) action=${effectiveDelivery.action} dest=${dest?.kind ?? "origin_thread"}`,
  );
  await deleteSession(sessionId);
}
...
function formatActionDescription(
  ...
  if (tool === "user-send-message") {
    ...
    if (content)
      lines.push(
        ``,
        `**Message:** ${content}${((params["content"] as string) ?? "").length > 300 ? "..." : ""}`,
      );
    return lines.join("\n");
  }
  ...
  if (tool === "spaces-create-ticket") {
    ...
    if (desc)
      lines.push(
        `**Description:** ${desc}${((params["description"] as string) ?? "").length > 300 ? "..." : ""}`,
      );
    return lines.join("\n");
  }
...
async function handleWebhook(req: Request, res: Response): Promise<void> {
  ...
  const { agentSlug: agentSlugFromUrl, spacesAppId: spacesAppIdFromUrl } = req.params as {
    agentSlug?: string;
    spacesAppId?: string;
  };
  if (spacesAppIdFromUrl) {
    ...
    if (eventType === "USER_MENTIONED" && agentRow.slug !== "digital-twin") {
      log.info(
        `Ignoring USER_MENTIONED on app/${spacesAppIdFromUrl} (${agentRow.slug}) — Twin handles this exclusively via digital-twin`,
      );
      res.json({ success: true });
      return;
    }
    ...
    if (legacyMatches.length > 1) {
      log.error(
        `[webhook] legacy slug route ambiguous after org-scoped slug flip agentSlug=${agentSlugFromUrl}; returning 404`,
      );
      res.status(404).json({ success: false, error: "Agent not found" });
      return;
    }
    ...
    if (eventType === "USER_MENTIONED" && agentSlugFromUrl !== "digital-twin") {
      log.info(
        `Ignoring USER_MENTIONED on /${agentSlugFromUrl} — Twin handles this exclusively via /webhook/digital-twin`,
      );
      res.json({ success: true });
      return;
    }
  ...
  const slash = rawSlash ?? (autoGoalEnabled ? parseSlashCommand(`/goal ${userText}`) : null);
  ...
  if (slash?.kind === "queueClear") {
    ...
    await spacesAppFetch(
      ...
      log.warn("Failed to post /queue clear reply", {
        error: err instanceof Error ? err.message : String(err),
      });
    });
    return;
  ...
  const goalCommand = slash && (slash.kind === "goalStart" || slash.kind === "goalStatus") ? slash : null;
  ...
  if (intercept.kind === "goalStatusReply" || intercept.kind === "goalCleared") {
    await spacesAppFetch(
      ...
      log.warn("Failed to post /goal control reply", {
        error: err instanceof Error ? err.message : String(err),
      });
    });
    return;
  ...
  if (fastTaskMatch) {
    ...
    if (payload.conversationId) {
      try {
        ...
        log.warn("Failed to set /fast override for /fast <task>", {
          error: err instanceof Error ? err.message : String(err),
        });
      }
    }
  ...
  const targetUserId =
    eventType === "USER_MENTIONED" && allMentionedIds.length > 0 ? allMentionedIds[0]! : payload.userId;
  ...
  if (QUEUE_ENABLED && eventType !== "USER_MENTIONED" && payload.conversationId && task) {
    ...
    if (!slot) {
      ...
      log.info(
        `[msg-queue] conv ${payload.conversationId} busy — queued eventId=${queuedMsg.eventId} (enqueued=${enq.enqueued} pos=${enq.position} deduped=${enq.deduped} full=${enq.full})`,
      );
      return;
    }
  }
  ...
  try {
    ...
    const dispatchRunForTarget = async (
      ...
      const userAgentConfig = agent.orgId
        ? await userAgentConfigRepository
            .findByUserAndAgent(targetUserId, agent.orgId, agent.slug)
            .catch(() => null)
        : null;
      const rawPersonalProvider = userAgentConfig?.provider;
      const personalProvider =
        rawPersonalProvider && rawPersonalProvider !== "spaces" ? rawPersonalProvider : undefined;
      ...
      const agentLevelProvider = (agentRow?.config as Record<string, unknown> | null)?.["provider"] as
        | string
        | undefined;
      ...
      const providerConfigs: Record<
        string,
        { apiKey: string; model: string; baseUrl?: string; authType?: string; reasoningEffort?: string }
      > = {};
      ...
      if (claudeCfg && claudeCfg.authType === "oauth_token") {
        ...
        const credRow =
          scope === "agent" ? agentCredsByProvider.get("claude") : credsByProvider.get("claude");
        ...
        if (credRow && ownerId) {
          ...
          const target =
            scope === "agent" && agentRow?.id
              ? agentCredRefreshTarget(
                  agentRow.id,
                  "claude",
                  credRow as { sharedCredentialId?: string | null },
                )
              : userCredRefreshTarget(
                  targetUserId,
                  "claude",
      ...
      const escalationCandidate: string | undefined =
        ...
        (agentLevelProvider &&
        providerConfigs[agentLevelProvider] &&
        providerScope[agentLevelProvider] === "agent"
          ? agentLevelProvider
          : undefined);
      ...
      log.info(
        `Provider resolution: mode=${providerAlwaysOn ? "always-on" : "kimi-first"} parent=${resolvedParentProvider ?? "spaces"} scope=${resolvedParentProvider ? (providerScope[resolvedParentProvider] ?? "fallback") : "platform"} creds=[${Object.keys(providerConfigs).join(",")}] order=[${runtimeProviderOrder.join(",")}] escalated=${escalatedProvider ?? "(none)"} userUpgrade=${userRequestedUpgrade} subagentOverrides=${JSON.stringify(subagentProviders)} subagentProviderMode=${subagentProviderMode}`,
      );
      ...
      if (userRequestedUpgrade && !task) {
        ...
        if (escalatedProvider) {
          ...
          await spacesAppFetch(
            ...
            log.warn("Failed to post /upgrade ack", {
              error: err instanceof Error ? err.message : String(err),
            });
          });
          ...
          await spacesAppFetch(
            "/chat/postMessage",
            {
              ...
              conversationId: payload.conversationId,
              markdownText:
                "⚠️ No premium provider configured for this agent. Ask an admin to add one in the agent's Provider Credentials.",
              userId: agent.spacesAppUserId,
              metadata: { contentFormat: "markdown" },
            },
            agent.appToken,
      ...
      if (liveSpaces) {
        ...
        log.info(
          `Resolved Spaces creds from live DB for user ${payload.userId} workspaceId=${liveSpaces.workspaceId}`,
        );
      } else {
        try {
          ...
          log.warn(
            `Failed to load user Spaces token for ${payload.userId}: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
      if (userSpacesToken && !userSpacesWorkspaceId) {
        ...
        if (workspaceId) {
          ...
          log.info(
            `Resolved Spaces workspaceId=${workspaceId} from user row for cached webhook auth user ${payload.userId}`,
          );
        }
      }
      ...
      const isAllowedAttachment = (att: WebhookAttachment): boolean => {
        ...
        if (
          /\.(txt|md|json|csv|yml|yaml|xml|log|pdf|xlsx|xlsm|docx|pptx|html|htm|xhtml|zip)$/.test(lowerName)
        )
          return true;
      ...
      const resultForwardUrl =
        ((payload as { metadata?: Record<string, unknown> }).metadata?.["resultForwardUrl"] as
          | string
          | undefined) || undefined;
      ...
      if (runAsTwin && QUEUE_ENABLED && payload.conversationId && task) {
        ...
        if (!twinConvSlotToken) {
          ...
          const enq = await enqueueMessage(queuedMsg);
          log.info(
            `[twin-queue] conv ${payload.conversationId} owner ${twinUserScope} busy — queued eventId=${queuedMsg.eventId} enqueued=${enq.enqueued} pos=${enq.position} deduped=${enq.deduped} full=${enq.full}`,
          );
          return;
        }
      }
      ...
      if (body.success && body.sessionId) {
        ...
        if (eventType !== "USER_MENTIONED" && !resultForwardUrl) {
          try {
            if (USE_EPHEMERAL_PROGRESS) {
              ...
              const placeholderRes = (await spacesAppFetch(
                "/chat/postMessage",
                {
                  channelId: payload.channelId,
                  conversationId: payload.conversationId,
            ...
            log.warn("Failed to publish initial agent progress signal", {
              error: err instanceof Error ? err.message : String(err),
            });
          }
        }
        ...
        if (pendingGoalStart) {
          await persistGoalStart({
            ...
            log.warn("Failed to persist /goal start — loop will not auto-continue", {
              error: err instanceof Error ? err.message : String(err),
            });
          });
        }
...
async function reconcileStoppedRuns(
  conversationId: string,
  fallbackAgentSlug: string,
): Promise<StopReconcileSummary> {
  ...
  for (const run of runningRuns) {
    try {
      ...
      await cancelRunRecovery(run.sessionId).catch((err) =>
        clog.warn(
          `[stop] cancelRunRecovery failed for ${run.sessionId}: ${err instanceof Error ? err.message : String(err)}`,
        ),
      );
      ...
      clog.info(
        `[stop] run ${run.sessionId} not cancellable (status=${body.status ?? "unknown"}) for conv ${conversationId}`,
      );
      ...
      clog.warn(
        `[stop] reconcile failed for run ${run.sessionId} conv ${conversationId}: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  for (const agentSlug of cleanedAgentSlugs) {
    await drainNextQueued(conversationId, agentSlug).catch((err) =>
      clog.warn(
        `[stop] orphan drain failed for conv ${conversationId}: ${err instanceof Error ? err.message : String(err)}`,
      ),
    );
  }
  return summary;
...
async function redispatchQueuedMessage(msg: QueuedMessage): Promise<void> {
  ...
  if (!queuedOrgId) {
    throw new Error(
      `/internal/run queued redispatch missing orgId for conv=${msg.conversationId} agent=${msg.agentSlug} user=${msg.userId}`,
    );
  }
  ...
  if (!agentRow?.spacesAppToken || !agentRow.spacesAppId) {
    throw new Error(
      `/internal/run queued redispatch missing Spaces app identity for conv=${msg.conversationId} agent=${msg.agentSlug} org=${queuedOrgId}`,
    );
  }
  ...
  const workspaceId =
    ...
    (agentRow.spacesAppUserId
      ? await getSpacesUserWorkspaceId(agentRow.spacesAppUserId).catch(() => null)
      : null);
  const traceId = createTraceId();
  ...
  if (!body?.success || !body.sessionId) {
    throw new Error(
      `/internal/run for queued message returned no sessionId conv=${msg.conversationId} agent=${msg.agentSlug}`,
    );
  }
  ...
  clog.info(
    `[msg-queue] registered queued session context sessionId=${body.sessionId} conv=${msg.conversationId} agent=${msg.agentSlug} workspaceId=${workspaceId ?? "(none)"}`,
  );
}
...
export async function drainNextQueued(
  ...
  try {
    ...
    clog.info(
      `[msg-queue] conv ${conversationId} agent ${agentSlug}${userScopeId ? ` owner ${userScopeId}` : ""}: dispatched queued eventId=${next.eventId}`,
    );
  } catch (err) {
    clog.warn(
      `[msg-queue] conv ${conversationId} agent ${agentSlug}: redispatch failed, releasing slot: ${err instanceof Error ? err.message : String(err)}`,
    );
    await releaseSlot(conversationId, agentSlug, token ?? undefined, userScopeId);
  }
}
...
async function redispatchTwinQueuedMessage(msg: QueuedMessage): Promise<void> {
  ...
  if (!res.ok || !body?.success || !body.sessionId) {
    throw new Error(
      `/internal/run for queued twin message returned no sessionId conv=${msg.conversationId} owner=${msg.userScopeId ?? "?"}`,
    );
  }
  ...
  clog.info(
    `[twin-queue] redispatched queued twin sessionId=${body.sessionId} conv=${msg.conversationId} owner=${msg.userScopeId ?? "?"}`,
  );
}
...
async function persistCallbackAttachments(
  ...
  for (const att of attachments) {
    try {
      ...
      clog.warn(
        `[webhook/result] failed to persist attachment ${att.fileName}: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
}
...
router.post(
  "/result",
  requireStrictS2S,
  requireResultToken((req) => (req.body as { sessionId?: string })?.sessionId),
  async (req: Request, res: Response) => {
    ...
    if (payload.status === "handoff") {
      const lastTurn =
        typeof (payload as { lastTurn?: unknown }).lastTurn === "number"
          ? (payload as { lastTurn: number }).lastTurn
          : undefined;
      clog.info(
        `[webhook/result] handoff callback session=${sessionId} conversation=${payload.conversationId ?? ""} agent=${payload.agentSlug ?? ""} lastTurn=${lastTurn ?? "unknown"}`,
      );
      const handoff = await handleRunHandoff(sessionId).catch((err) => {
        clog.warn(
          `[webhook/result] handoff re-dispatch failed session=${sessionId}:`,
      ...
      if (handoff) {
        clog.info(
          `[webhook/result] handoff re-dispatched root=${handoff.rootSessionId} newSession=${handoff.newSessionId}`,
        );
        ...
        let handoffCtx = await resolveSessionContext(
          sessionId,
          payload.conversationId,
          payload.agentSlug,
          payload.userId,
        ...
        if (!handoffCtx && sessionId) {
          ...
          const metadata = storedRun?.metadata as
    ...
    try {
      if (sessionId && payload.status === "failed" && payload.error === "sse stream broken") {
        ...
        if (existingRun && existingRun.status !== "running") {
          clog.info(
            `[webhook/result] duplicate broken-SSE terminal for ${sessionId}; existing status=${existingRun.status}, skipping`,
          );
          skipQueueDrain = true;
          return;
        }
      ...
      if (ctx?.responseMode === "approval" && payload.status === "completed") {
        ...
        if (ctx.conversationId && ctx.agentSlug && ctx.agentOrgId && ctx.mentionedUserId) {
          try {
            ...
            clog.warn(
              `[webhook/result] Twin: failed to persist outcome chat message for ${sessionId}: ${e instanceof Error ? e.message : String(e)}`,
            );
          }
        }
        ...
        if (sessionId) {
          ...
          await handleRunCompletion(sessionId, "completed").catch((err) => {
            clog.warn(
              `[webhook/result] Twin: failed to settle run recovery for ${sessionId}:`,
        ...
        if (payload.twinDelivery?.action === "ignore") {
          clog.info(
            `[webhook/result] Digital Twin chose to ignore — dropping, no DM/post, session ${sessionId}`,
          );
          await deleteSession(sessionId);
          ...
          clog.info(
            `[webhook/result] Digital Twin stayed silent — no twin_deliver delivery (fail-closed), session ${sessionId}`,
          );
          await deleteSession(sessionId);
        }
        return;
      ...
      if (ctx?.agentSlug && ctx.agentOrgId) {
        try {
          ...
          const replyOpts = (
            agentRow?.config as { replyOptions?: { includeCitations?: boolean } } | undefined
          )?.replyOptions;
      ...
      if (payload.status === "completed") {
        await handleRunCompletion(sessionId, "completed").catch((err) => {
          clog.warn(
            `[webhook/result] Failed to mark ${sessionId} completed in run recovery:`,
            err instanceof Error ? err.message : err,
          );
        });
      }
      ...
      if (isSessionLockedFailure) {
        ...
        skipQueueDrain = true;
        const recoveryFailure = await handleRunCompletion(sessionId, "failed", "session_locked").catch(
          (err) => {
            clog.warn(
              `[webhook/result] Failed to process ${sessionId} lock contention in run recovery:`,
              err instanceof Error ? err.message : err,
            );
            return null;
          },
        ...
        if (!recoveryFailure && ctx?.responseMode === "conversation" && ctx.conversationId && ctx.agentSlug) {
          ...
          clog.info(
            `[webhook/result] Session ${sessionId}: session locked; queued from session context enqueued=${enq.enqueued} deduped=${enq.deduped} full=${enq.full} pos=${enq.position}`,
          );
        } else {
          clog.info(
            `[webhook/result] Session ${sessionId}: session locked; queued via run recovery without user-facing failure`,
          );
        }
      ...
      if (payload.status === "failed" || payload.status === "cancelled") {
        const recoveryFailure = await handleRunCompletion(
          sessionId,
          "failed",
          payload.error ?? payload.status,
        ).catch((err) => {
          clog.warn(
            `[webhook/result] Failed to process ${sessionId} failure in run recovery:`,
            err instanceof Error ? err.message : err,
          );
          return null;
        });
        ...
        if (recoveryFailure?.retried) {
          clog.info(
            `[webhook/result] Session ${sessionId}: retry queued (${recoveryFailure.retriesUsed}/${recoveryFailure.maxRetries})`,
          );
          return;
        }
      }
      ...
      if (externalResultCallback) {
        await deleteSession(sessionId);
        await sendStoredExternalResultCallback(externalResultCallback, {
          sessionId,
      ...
      if (ctx?.slackDelivery) {
        ...
        await deliverSlackResult({
          ...
          clog.warn(
            `[webhook/result] Slack delivery failed for session ${sessionId}: ${err instanceof Error ? err.message : String(err)}`,
          );
        });
        return;
      }
      ...
      if (payload.status !== "completed") {
        ...
        if (ctx?.resultForwardUrl) {
          await forwardResult(
            ctx.resultForwardUrl,
            { sessionId, status: "failed", ...(payload.error ? { error: payload.error } : {}) },
            "",
          );
          return;
        }
        ...
        if (payload.status === "failed" && ctx?.agentSlug && ctx.agentOrgId) {
          try {
            ...
            const userChainRow = ctx.agentOrgId
              ? await userAgentConfigRepository.findByUserAndAgent(
                  ctx.senderId,
                  ctx.agentOrgId,
                  ctx.agentSlug,
                )
            ...
            if (chain?.onFailure?.triggerAgent) {
              ...
              const failureAgentRow = await agentRepository.findBySlug(
                chain.onFailure.triggerAgent,
                ctx.agentOrgId,
              );
              ...
              if (!runRes.ok) {
                clog.error(`[webhook/result] Failure chain trigger HTTP ${runRes.status}`);
                return;
              }
              ...
              if (
                runBody.success &&
                runBody.sessionId &&
                failureAgentRow?.spacesAppToken &&
                failureAgentRow.spacesAppId
              ) {
        ...
        if (
          ...
          try {
            ...
            if (agentRow) {
              ...
              if (!isAlwaysOn) {
                const agentCreds = await agentProviderCredentialsRepository
                  .listByAgent(agentRow.id)
                  .catch(() => []);
                ...
                const order: string[] = Array.isArray(rawOrder)
                  ...
                  : [];
                const legacy = (agentRow.config as Record<string, unknown> | null)?.["provider"] as
                  | string
                  | undefined;
                const candidate =
                  order.find(hasCreds) ??
                ...
                if (candidate) {
                  const flow = withSpacesAppId(
                    buildPromoteProviderFlow(candidate, {
                      agentSlug: ctx.agentSlug,
                      channelId: ctx.channelId,
                      conversationId: ctx.conversationId,
                      userId: ctx.senderId,
                  ...
                  await spacesAppFetch(
                    ...
                    clog.warn(
                      "[webhook/result] failed to post promote-provider prompt:",
                      err instanceof Error ? err.message : err,
                    );
                  });
                  failureSurfaced = true;
                  clog.info(
                    `[webhook/result] posted promote-provider prompt for conv ${ctx.conversationId} (provider=${candidate})`,
                  );
                }
              }
            }
            ...
            clog.warn(
              "[webhook/result] promote-provider prompt error (non-fatal):",
              err instanceof Error ? err.message : err,
            );
          }
        }
        ...
        if (
          ...
          await spacesAppFetch(
            ...
            clog.warn(
              "[webhook/result] failed to post failure notice:",
              err instanceof Error ? err.message : err,
            ),
          );
          clog.info(
            `[webhook/result] posted generic failure notice for conv ${ctx.conversationId} (quota=${isQuota})`,
          );
        }
      ...
      const runOwnerId =
        ctx.targetUserId ?? (ctx.responseMode === "approval" ? ctx.mentionedUserId : ctx.senderId);
      ...
      if (ctx.resultForwardUrl) {
        if (resultWithCitations.trim() && ctx.conversationId && ctx.agentSlug && ctx.agentOrgId) {
          ...
          chatMessageRepository
            ...
            .catch((e) =>
              log.warn("Failed to save assistant ChatMessage", {
        ...
        log.info(
          `[webhook/result] mention-resolve gate: resolveMentions=${!!ctx.resolveMentions} appToken=${!!ctx.appToken} hasText=${!!forwardText.trim()} pending=${payload.pendingResponses?.length ?? 0} usedPending=${!resultWithCitations.trim() && !!forwardText.trim()} sessionId=${sessionId}`,
        );
        if (ctx.resolveMentions && ctx.appToken && forwardText.trim()) {
          try {
            ...
            log.warn(
              `mention resolution failed — forwarding raw text: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        }
      ...
      if (ctx.responseMode === "conversation" && !ctx.channelId && !ctx.conversationId) {
        log.warn(
          `No channel and no resultForwardUrl for ${sessionId} — nowhere to deliver; skipping channel post (no retry).`,
        );
        await deleteSession(sessionId);
        return;
      }
      ...
      if (USE_EPHEMERAL_PROGRESS) {
        spacesAppFetch(
          ...
          log.warn("Failed to clear agent progress signal", {
            error: err instanceof Error ? err.message : String(err),
          }),
        );
      }
      ...
      if (
        ...
        try {
            ...
            if (prevCard?.messageId) {
              try {
                ...
                await spacesAppFetch(
                  "/chat/updateMessage",
                  {
                    messageId: prevCard.messageId,
                    flowJSON: supersededFlow,
                    userId: ctx.spacesAppUserId,
                    channelId: ctx.channelId,
                  },
                  token,
          ...
          const planMessageId =
            planResp?.messageId ?? planResp?.id ?? planResp?.data?.messageId ?? planResp?.data?.id;
          if (planMessageId) {
            ...
            log.warn(
              `Plan card posted but no messageId returned — Turn 2 may post a duplicate card (conv=${ctx.conversationId})`,
              { planResp },
            );
          }
          ...
          if (trivial) {
            ...
            if (runBody?.success && runBody.sessionId) {
              ...
              log.info(
                `Auto-dispatched trivial plan Turn 2 session=${runBody.sessionId} conv=${ctx.conversationId}`,
              );
            } else {
      ...
      if (!resultWithCitations.trim() && !payload.pendingResponses?.length) {
        ...
        try {
          ...
          const sorryText =
            payload.emptyReason === "provider_capacity"
          ...
          log.error("Failed to send empty-result notice", {
            error: err instanceof Error ? err.message : String(err),
          });
        }
        return;
      ...
      try {
        ...
        const resolvePendingMentions = async (text: string): Promise<string> => {
          ...
          try {
            ...
            log.warn(
              `pending-response mention resolution failed — posting raw: ${err instanceof Error ? err.message : String(err)}`,
            );
            return text;
          }
        };
        ...
        try {
          ...
          if (ctx.conversationId && ctx.senderId) {
            const priorRuns = await agentRunRepository.listByConversation(ctx.conversationId, ctx.senderId, {
              limit: 50,
            });
          ...
          log.warn(
            `Failed to assemble citation invocations for baking: ${e instanceof Error ? e.message : String(e)}`,
          );
        }
        ...
        if (payload.pendingResponses?.length) {
          if (ctx.responseMode === "approval") {
            ...
            const dmResult = (await spacesAppFetch(
              "/channel/openDm",
              {
            ...
            const twinFlow = withSpacesAppId(
              buildTwinApprovalFlow({
          ...
          if (payload.attachments?.length) {
            ...
            try {
              ...
              log.info(
                `Copilot: uploaded ${prepared.attachments.length} attachment(s) with response(s) in thread ${ctx.conversationId}`,
              );
            } catch (err) {
              ...
              log.info(
                `Copilot: posted text-only fallback after attachment upload failure in thread ${ctx.conversationId}`,
              );
            }
          } else {
            ...
            log.info(
              `Copilot: posted ${payload.pendingResponses.length} response(s) in thread ${ctx.conversationId}`,
            );
          }
          ...
          if (
            ...
            chatMessageRepository
              ...
              .catch((e) =>
                log.warn("Failed to save pending-response assistant ChatMessage", {
                  error: e instanceof Error ? e.message : String(e),
                }),
              );
          }
          ...
          const copilotPendingActions = (payload as { pendingActions?: Array<Record<string, unknown>> })
            .pendingActions;
          if (copilotPendingActions?.length) {
            ...
            for (const action of copilotPendingActions) {
              ...
              if (targetValidation.error) {
                log.info(
                  `[webhook/result] skipped write approval card tool=${String(action["tool"] ?? "")}: ${targetValidation.error}`,
                );
                continue;
              }

              const actionDesc = formatActionDescription(
              ...
              const writeFlow = withSpacesAppId(
                buildWriteApprovalFlow(actionDesc, {
              ...
              if (
                action["tool"] === "spaces-memory-create" &&
                (action["params"] as Record<string, unknown>)?.["content"]
              ) {
            ...
            log.info(
              `Copilot: posted ${copilotApprovalCardsSent}/${copilotPendingActions.length} write action approval(s)`,
            );
          }
        ...
        if (ctx.responseMode === "approval") {
          ...
          clog.warn(
            `[webhook/result] approval path reached post-processing with no delivery — staying silent, session ${sessionId}`,
          );
          await deleteSession(sessionId);
          return;
        } else {
          ...
          const threadCitationMeta = buildThreadCitationMeta(citationInvocations, resultWithCitations);
          const convMetadata = {
          ...
          const agentWsId =
            !senderAuth?.token && ctx.spacesAppUserId
          ...
          if (prepared.attachments.length > 0) {
            ...
            try {
              ...
              log.info(
                `Agent ${ctx.agentSlug}: replied with ${prepared.attachments.length} attachment(s)` +
                  (agentResult.length > MAX_MESSAGE_CHARS
                    ? ` (PDF fallback, original ${agentResult.length} chars)`
                    : ""),
              );
            } catch (err) {
              ...
              log.warn(`Attachment upload failed for ${ctx.agentSlug} — falling back to text-only reply`, {
                error: err instanceof Error ? err.message : String(err),
              });
              const fileNote = `⚠️ _Couldn't attach ${prepared.attachments.length} file(s) (upload failed)._`;
              const fallbackText = prepared.text?.trim()
              ...
              log.info(
                `Agent ${ctx.agentSlug}: posted text-only fallback after attachment upload failure in thread ${ctx.conversationId}`,
              );
            }
          } else {
            ...
            if (!USE_EPHEMERAL_PROGRESS && ctx.progressMessageId) {
              try {
                ...
                log.info(
                  `Updated placeholder ${ctx.progressMessageId} with final result for ${ctx.agentSlug}`,
                );
                posted = true;
              } catch (err) {
                log.warn("Failed to update placeholder with final result — falling back to fresh post", {
                  error: err instanceof Error ? err.message : String(err),
                });
              }
            }
            if (!posted) {
              log.info(
                `Posting result: channelId=${ctx.channelId} conversationId=${ctx.conversationId} resultLen=${prepared.text.length} userId=${ctx.spacesAppUserId}`,
              );
              await spacesAppFetch(
                "/chat/postMessage",
                {
                ...
                sizeBytes:
                  typeof a.data === "string"
                    ? // base64 → bytes: ceil(len * 3/4), minus padding "="s
                      Math.max(
                        0,
                        Math.floor((a.data.length * 3) / 4) - (a.data.match(/=+$/)?.[0]?.length ?? 0),
                      )
                    : 0,
              }));
        ...
        if (
          ...
          const looksSoftRefusal =
            SOFT_REFUSAL_RE.test(result) || (result.length < 250 && toolsUsedCount === 0);
          if (looksSoftRefusal) {
            try {
              const agentRow = await agentRepository.findBySlug(ctx.agentSlug, ctx.agentOrgId);
              if (agentRow) {
                ...
                if (candidate) {
                  ...
                  await spacesAppFetch(
                    ...
                    log.warn("Failed to post promote-provider prompt (soft refusal)", {
                      error: err instanceof Error ? err.message : String(err),
                    });
                  });
                  log.info(
                    `Posted promote-provider prompt for conv ${ctx.conversationId} (soft refusal, provider=${candidate})`,
                  );
                }
              }
            } catch (err) {
              log.warn("promote-provider prompt (soft refusal) error (non-fatal)", {
                error: err instanceof Error ? err.message : String(err),
              });
            }
          }
        ...
        const pendingQuestions = (
          payload as { pendingQuestions?: Array<{ questionId: string; question: string; options: string[] }> }
        ).pendingQuestions;
        if (pendingQuestions?.length) {
          for (const q of pendingQuestions) {
            const questionFlow = withSpacesAppId(
              buildUserQuestionFlow(q.question, q.options, {
        ...
        if (goalSuggestion && goalSuggestion.condition?.trim() && goalSuggestion.rationale?.trim()) {
          ...
          const goalFlow = withSpacesAppId(
            buildGoalSuggestionFlow(safeRationale, {
        ...
        const pendingActionsPayload = (payload as { pendingActions?: Array<Record<string, unknown>> })
          .pendingActions;
        if (pendingActionsPayload?.length) {
          ...
          log.info(
            `Sent ${approvalCardsSent}/${pendingActionsPayload.length} write action approval(s) to ${ctx.senderId}`,
          );
        }
        ...
        if (ctx.agentSlug && ctx.agentOrgId) {
          try {
            ...
            const binding = await agentChainWorkflowRepository.findActiveWorkflowForChannel(
              ctx.channelId,
              rootAgentSlug,
              ctx.senderId,
            );
            ...
            const maxDepth = isCycleWorkflow ? 3 : Math.max(1, Math.min(workflow.maxDepth ?? 6, 12));
            if (currentDepth >= maxDepth) {
              ...
              log.info(
                `Chain: max depth ${maxDepth} reached for workflow ${binding.workflowId} (cycle=${isCycleWorkflow})`,
              );
              return;
            }
            ...
            const selected = await selectNextWorkflowEdge(
              workflow,
              ctx.agentSlug,
              toolsUsed,
              resultText,
            ...
            if (!runRes.ok) {
              log.error(`Chain trigger HTTP ${runRes.status}`);
              return;
            }
            ...
            if (
              runBody.success &&
              runBody.sessionId &&
              targetAgentRow.spacesAppToken &&
              targetAgentRow.spacesAppId
            ) {
              ...
              log.info(
                `Chain: ${ctx.agentSlug} → ${targetAgentSlug} (step ${currentDepth + 1}/${maxDepth}, workflow ${binding.workflowId})`,
              );
              await spacesAppFetch(
                "/chat/postMessage",
                {
            ...
            log.error("Chain trigger failed (non-fatal):", {
...
router.post("/progress", requireStrictS2S, async (req: Request, res: Response) => {
  ...
  if ((req.body as { kind?: string }).kind === "plan") {
    ...
    renderPlanCard(sessionId, todos, conversationId, agentSlug).catch((e) =>
      clog.warn(
        `[webhook/progress] renderPlanCard failed for ${sessionId}:`,
  ...
  await touchRunRecovery(sessionId).catch((err) => {
    clog.warn(
      `[webhook/progress] touchRunRecovery failed for ${sessionId}:`,
      err instanceof Error ? err.message : err,
    );
  });
  ...
  if (toolInvocation) {
    agentRunRepository.appendToolInvocation(sessionId, toolInvocation).catch((e) => {
      clog.warn(
        `[webhook/progress] appendToolInvocation failed for ${sessionId}:`,
        e instanceof Error ? e.message : e,
      );
    });
  ...
  if (sandboxPreviewUrl && sandboxId) {
    ...
    try {
      ...
      log.warn("Failed to announce sandbox preview", {
        error: err instanceof Error ? err.message : String(err),
      });
    }
    return;
  ...
  try {
    ...
    log.warn("Failed to publish agent progress signal", {
      error: err instanceof Error ? err.message : String(err),
    });
  }
});
...
async function proxyFlowAction(req: Request, res: Response, headers: Record<string, string>): Promise<void> {
  ...
  try {
    ...
    clog.error(
      `[webhook/flow-action-proxy] fetch failed: ${err instanceof Error ? err.message : String(err)}`,
    );
  ...
  res
    .status((proxyRes as unknown as { status: number }).status)
