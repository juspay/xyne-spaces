 ...
 */
router.post("/:userId/oauth/mailerlite/authorize", asyncHandler(async (req, res) => {
 ...
 */
router.post("/:userId/oauth/mailerlite/callback", asyncHandler(async (req, res) => {
  const { userId } = req.params as { userId: string };
  ...
  ok(res, { message: "MailerLite account connected successfully" });
}));
