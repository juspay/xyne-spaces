 ...
 */
router.post("/:userId/oauth/calendly/authorize", asyncHandler(async (req: Request<{ userId: string }>, res: Response, next?: NextFunction) => {
 ...
 */
router.post("/:userId/oauth/calendly/callback", asyncHandler(async (req: Request<{ userId: string }>, res: Response, next?: NextFunction) => {
  const { userId } = req.params;
  ...
  if (!tokenRes.ok) {
    ...
    throw new HttpError(502, "Calendly token exchange failed");
  }
  ...
  ok(res, { message: "Calendly account connected successfully" });
}));
