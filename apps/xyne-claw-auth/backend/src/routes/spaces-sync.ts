...
router.post("/user", async (req: Request, res: Response) => {
  try {
    ...
    const spacesWorkspaceId = stringField(body, "spacesWorkspaceId");
    const spacesOrgId = stringField(body, "spacesOrgId");
    // Spaces users are workspace memberships. This is the stable identity for
    // the person inside the org, shared by all of their workspace-user rows.
    // Optional during rollout so already-deployed Spaces producers continue to
    // sync; once all producers send it, make this required.
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const exactUser = await tx.user.findUnique({
        ...
        select: { id: true, email: true, orgId: true, spacesOrgMemberId: true },
      });
      const memberUser = spacesOrgMemberId
      ...
      if (memberUser && exactUser && memberUser.id !== exactUser.id) {
        throw new SyncError(
          409,
          `Spaces org member ${spacesOrgMemberId} resolves to Claw user ${memberUser.id}, ` +
            `but workspace user ${spacesUserId} resolves to ${exactUser.id}`,
        );
      }
      if (memberUser && emailUser && memberUser.id !== emailUser.id) {
        throw new SyncError(
          409,
          `Spaces org member ${spacesOrgMemberId} resolves to Claw user ${memberUser.id}, ` +
      ...
      for (const candidate of [exactUser, emailUser]) {
        if (spacesOrgMemberId && candidate?.spacesOrgMemberId && candidate.spacesOrgMemberId !== spacesOrgMemberId) {
          throw new SyncError(
            409,
            `Claw user ${candidate.id} is already linked to Spaces org member ${candidate.spacesOrgMemberId}; ` +
              `refusing reassignment to ${spacesOrgMemberId}`,
          );
        }
      }

      const role = mapOrgRole(optionalString(body, "role"));
      const existingCanonicalUser = memberUser ?? exactUser ?? emailUser;
      const canonicalUser = existingCanonicalUser ?? await tx.user.create({
        // Claw owns its primary key. Spaces' workspace membership id belongs
        // exclusively in UserSurfaceIdentity; the stable external identity is
        // spacesOrgMemberId.
        data: {
      ...
      if (existingCanonicalUser) {
        await tx.user.update({
          ...
          data: { email, name, ...(spacesOrgMemberId ? { spacesOrgMemberId } : {}) },
        });
      }
