 ...
 */
router.post("/:userId/oauth/google/authorize", asyncHandler(async (req: Request<{ userId: string }>, res: Response, next?: NextFunction) => {
 ...
 */
router.post("/:userId/oauth/google/callback", asyncHandler(async (req: Request<{ userId: string }>, res: Response, next?: NextFunction) => {
  const { userId } = req.params;
  ...
  ok(res, { message: "Google account connected successfully" });
}));
