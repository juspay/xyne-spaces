 ...
 */
router.post("/:userId/oauth/wix/authorize", asyncHandler(async (req, res) => {
 ...
 */
router.post("/:userId/oauth/wix/callback", asyncHandler(async (req, res) => {
  const { userId } = req.params as { userId: string };
  ...
  if (!tokenRes.ok) {
    ...
    throw new HttpError(502, "Wix token exchange failed");
  }
  ...
  ok(res, { message: "Wix account connected successfully" });
}));
