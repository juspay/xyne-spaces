...
export async function ensureUserExists(
  ...
  // Do not return early for a legacy Claw user. Reading their live Spaces
  // profile lets this path stamp spacesOrgMemberId and create the missing
  // workspace UserSurfaceIdentity on their next authenticated request.
  const existing = await prisma.user.findUnique({ where: { id: userId } });
  if (existing?.spacesOrgMemberId) {
    const identity = await prisma.userSurfaceIdentity.findFirst({
      where: {
        surfaceId: "spaces",
  ...
  if (memberId && existing?.spacesOrgMemberId && existing.spacesOrgMemberId !== memberId) {
    log.error(
      `[users-jit] user ${spacesUser.id} is already linked to Spaces orgMemberId=${existing.spacesOrgMemberId}, ` +
        `received ${memberId}; refusing reassignment`,
    );
    return false;
  }
  const existingByMember = memberId
    ? await prisma.user.findUnique({
        where: { orgId_spacesOrgMemberId: { orgId, spacesOrgMemberId: memberId } },
        select: { id: true, email: true },
      })
    : null;
  const existingInOrg = await prisma.user.findFirst({
    where: { orgId, email: { equals: email, mode: "insensitive" } },
    select: { id: true, spacesOrgMemberId: true },
  });

  if (existingByMember && existingInOrg && existingByMember.id !== existingInOrg.id) {
    log.error(
      `[users-jit] identity conflict: Spaces orgMemberId=${memberId} resolves to Claw user ${existingByMember.id}, ` +
        `but email=${email} resolves to ${existingInOrg.id} in org=${orgId}`,
    );
    return false;
  }
  if (memberId && existingInOrg?.spacesOrgMemberId && existingInOrg.spacesOrgMemberId !== memberId) {
    log.error(
      `[users-jit] identity conflict: email=${email} in org=${orgId} is already stamped ` +
        `with Spaces orgMemberId=${existingInOrg.spacesOrgMemberId}, received ${memberId}`,
    );
    return false;
  }

  let canonicalUserId = existingByMember?.id ?? existingInOrg?.id;

  if (canonicalUserId) {
    await prisma.user.update({
      where: { id: canonicalUserId },
      data: { email, name: spacesUser.name, ...(memberId ? { spacesOrgMemberId: memberId } : {}) },
    });
    log.info(
    ...
    const created = await prisma.user.create({
      data: {
        ...
        orgId,
        ...(memberId ? { spacesOrgMemberId: memberId } : {}),
      },
      select: { id: true },
    });
