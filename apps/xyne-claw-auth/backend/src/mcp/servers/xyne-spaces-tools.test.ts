...
describe("spaces-sdlc-mutate-artifact schema", () => {
  ...
  it.each(["PRD", "TECH_DOC"])("updates %s artifacts by canonical canvas ID", async artifactType => {
    const tool = await artifactMutationTool();
    const variants = tool.inputSchema.oneOf as Array<{
      properties: { artifactType?: { const?: string }; action?: { const?: string } };
      required: string[];
    }>;
    const update = variants.find(candidate =>
      candidate.properties.artifactType?.const === artifactType &&
      candidate.properties.action?.const === "update"
    );

    expect(update?.required).toContain("canvasId");
    expect(update?.required).not.toContain("viewAccessId");
  });

  it("sends the canonical canvas ID when updating an interactive PRD", async () => {
    vi.clearAllMocks();
    mocks.spacesFetch.mockResolvedValueOnce({
      success: true,
      artifact: { canvasId: "canvas-1", kind: "PRD" },
    });
    const tool = await artifactMutationTool();
    ...
    expect(mocks.spacesFetch).toHaveBeenCalledWith(
      "/api/sdlc/claw/artifacts/update",
      expect.objectContaining({
        method: "POST",
        body: expect.stringContaining('"canvasId":"canvas-1"'),
      }),
    );
  });
  ...
  it("allows interactive PRD creation without a workflow execution", async () => {
    await expect(validateWriteAction("xyne-spaces", "spaces-sdlc-mutate-artifact", {
      repoId: "repo-1",
      artifactType: "PRD",
      action: "create",
      title: "Search experience",
      markdown: "# Search experience",
    }, {})).resolves.toBeNull();
  });

  it("requires the canonical canvas ID for PRD updates", async () => {
    await expect(validateWriteAction("xyne-spaces", "spaces-sdlc-mutate-artifact", {
      repoId: "repo-1",
      artifactType: "PRD",
      action: "update",
      markdown: "# Updated",
    }, {})).resolves.toBe("canvasId is required for update");
  });
