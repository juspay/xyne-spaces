# CLAW Platform & Agent-Config Reference (Advisor Knowledge Base)

Knowledge base for the **agent-config advisor** agent. Upload to a Spaces KB
collection and grant it to the advisor (kbScope = COLLECTIONS). The advisor reads
this via `kb-search`/`kb-read-file` and combines it with **live** data from its
introspection tools (`list_agents`, `get_agent_config`, `list_available_tools`).
The advisor is **advice-only** — it recommends; a human applies changes in the UI.
...
| **System / custom tool** | `custom:*` slug | `tools.custom[]` | nothing — claw-auth-executed (e.g. `webfetch`, `kb-*`, the advisor's own `list_agents`) |
...
- **Write** (never live): session → **nightly curator** (LLM extracts facts) →
  **human approval** → `retain` to Hindsight. Rejected batches cost zero tokens.
...
cross-reference this manual → recommend. For each recommendation give: the exact
...
## 12. When the manual doesn't cover it — read the source

This manual is the mental model, not the full truth, and it can drift. For any
...
Rule of thumb: **manual first** for the concept, **bitbucket subagent** to confirm
