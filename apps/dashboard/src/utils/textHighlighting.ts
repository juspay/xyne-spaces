...
class TextTokenizer {
  public static tokenize(text: string, caseSensitive: boolean = false): string[] {
    ...
    const filtered: string[] = (removeStopwords as (words: string[]) => string[])(words);
...
class OrderedWindowFinder {
  public findMinimumWindow(tokenPositions: number[][]): { start: number; end: number } | null {
    ...
    for (let sentenceId = 0; sentenceId < tokenPositions.length; sentenceId++) {
      const arr = tokenPositions[sentenceId];
      if (arr && arr.length > 0) {
        // Only include tokens that have occurrences
        for (const pos of arr) merged.push({ pos, sentenceId });
      }
    }
    ...
    for (let r = 0; r < merged.length; r++) {
      ...
      while (have === need) {
        ...
        if (bestL === null || posR - posL < bestR! - bestL) {
          bestL = posL;
          bestR = posR;
        }
  ...
  public search(text: string): Map<number, number[]> {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      for (const patternIndex of current.output) {
        const startPos = i - (this.patterns[patternIndex]?.length ?? 0) + 1;
...
const mergeCloseMatches = (
  ...
  for (let i = 1; i < sortedMatches.length; i++) {
    ...
    if (gap <= 128) {
      ...
      currentMatch = {
        startIndex: nextMatch.startIndex,
        endIndex: nextMatch.endIndex,
...
export const findHighlightMatches = (
  ...
  try {
    ...
    const bestWindow = windowFinder.findMinimumWindow(tokenPositions);
    ...
    for (let i = 0; i < queryTokens.length; i++) {
      ...
      const windowPositions = positions.filter(
        pos => pos >= bestWindow.start && pos <= bestWindow.end,
      );
    ...
    const mergedMatches = mergeCloseMatches(
      matches.map(match => ({
        ...match,
