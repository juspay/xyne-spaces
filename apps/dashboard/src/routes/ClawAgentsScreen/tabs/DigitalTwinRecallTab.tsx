  ...
  return (
    <div className='flex max-w-5xl flex-col gap-7'>
      ...
      <form onSubmit={submit} className='border-y dt-rule py-6'>
          ...
          <div className='mt-2 grid gap-px bg-[var(--dt-rule)] sm:grid-cols-3'>
            {BUDGETS.map(option => (
              <label
                ...
                className={
                  budget === option.value
                    ? 'dt-paper-raised flex min-h-20 cursor-pointer gap-3 bg-[var(--dt-accent-soft)] p-3'
                    : 'dt-paper-raised flex min-h-20 cursor-pointer gap-3 p-3'
                }
              >
                <input
                  type='radio'
        ...
        <Button
          type='submit'
          className='dt-control mt-5 bg-[var(--dt-accent)] text-[var(--dt-on-accent)] hover:bg-[var(--dt-accent)] hover:opacity-90'
          disabled={!query.trim()}
          ...
          {results.length === 0 ? (
            ...
            <ol>
              {results.map((result, index) => (
                <li key={result.id ?? index} className='border-b dt-rule py-5'>
