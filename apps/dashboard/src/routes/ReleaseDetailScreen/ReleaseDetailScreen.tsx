...
const ReleaseDetailScreen = (): ReactElement => {
  ...
  // Version-mode releases carry no commit range, so commit analysis — and the
  // release notes it produces — never run for them.
  const isVersionRelease = releaseBoard?.releaseTrackingMode === ReleaseTrackingMode.VERSION;
              ...
              <Tabs.Content value='releasenotes' className='mt-6 outline-none'>
                ...
                {analysisCanvasId ? (
                  ...
                  <div className='text-center py-8 bg-muted rounded-lg border border-dashed border-border'>
                    <p className='text-sm text-muted-foreground'>
                      {isVersionRelease
                        ? 'Release notes are generated by commit analysis, which version-tracked releases do not run.'
                        : 'No release notes yet. They appear here once commit analysis has run for this release — use “Re-run Analysis” on the Dev Tickets tab to generate them.'}
                    </p>
                  </div>
                )}
              </Tabs.Content>
