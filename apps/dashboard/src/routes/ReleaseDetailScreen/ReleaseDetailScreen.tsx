...
const ReleaseDetailScreen = (): ReactElement => {
  ...
  const [timelineEvents] = useCachedQuery(
    queries.releaseEventsByReleaseId({ releaseId: releaseTicketId ?? '' }),
  ...
  // Selected column keys are persisted on the release ticket's metadata so every
  // viewer sees the same columns (real-time via Zero).
          ...
          <div className='bg-background rounded-lg shadow-sm border border-border p-6 space-y-6'>
            ...
            <Tabs.Root value={activeTab} onValueChange={v => setActiveTab(v as TabValue)}>
                        ...
                        {devTicketRows.map(row => {
                          return (
                            <tr key={row.internalTicketId} className='border-t border-border'>
                              ...
                              {selectedColumns.map(col => (
                                <td key={col.key} className='px-4 py-2 text-muted-foreground'>
              ...
              {/* Timeline tab — audit log of everything that happened on this
                  ...
                  captures, system events, canvas publishes). Consecutive
                  identical-name events are grouped to keep the feed scannable. */}
