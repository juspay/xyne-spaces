export interface SdlcArtifactCreationPromptInput {
  ...
  folderId: string;
  title: string;
  /** Absent on a hub that covers no repository; the hub itself is then the scope. */
...
export function buildSdlcArtifactCreationPrompt(input: SdlcArtifactCreationPromptInput): string {
  ...
  const scope = JSON.stringify(input.repositoryName ?? input.hubName ?? '');
  ...
  const relatedClause =
    input.relatedArtifacts && input.relatedArtifacts.length > 0
      ...
      : '';
  const request =
    `Create a ${typeLabel} titled ${title} in ${
      input.repositoryName ? 'repository' : 'SDLC hub'
    } ${scope}${trackClause}. ` +
