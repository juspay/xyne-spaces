 ...
 *
 * Ported from xyne-claw-auth/frontend/src/v3/lib/agentCategory.ts (agent half
 * only — the MCP derivation is omitted since the dashboard has no McpServer
 * type here). Keep the taxonomy in sync with that source of truth.
 */

import type { Agent } from './clawAuthAgentTypes';
import type { McpServer } from './clawMcpTypes';
import type { Skill } from './clawSkillsTypes';

export interface AgentCategoryDefinition {
  /** Stable id — what would persist to a DB column if added later. */
  readonly id: string;
  /** Display label rendered in section headers / filters. */
  ...
  /** Tailwind class string for the per-row chip. */
  readonly chipClass: string;
  /**
   * Regex matched against a haystack of the agent's tool slugs/names, skill
   * names, agent name, and description. `null` means "no auto-match" (used for
...
export const AGENT_CATEGORIES = [
  ...
  },
  {
    ...
    match: /\bdrive\b|\bdocs\b|sheets|dropbox|\bbox\b|\bpdf\b|files|onedrive|sharepoint|airtable/i,
  },
  {
    id: 'other',
    label: 'Other',
...
/** Lookup helper. Always returns a definition (ids are constrained to the registry). */
export function getCategoryDefinition(id: AgentCategoryId): AgentCategoryDefinition {
  return AGENT_CATEGORIES.find(c => c.id === id)!;
}

/**
 * Pick the category whose rules match the haystack most strongly. Ties broken
 * by registry order (earlier wins). Empty/whitespace input → "other".
 */
function deriveCategoryFromHaystack(haystack: string): AgentCategoryId {
  ...
  for (const cat of AGENT_CATEGORIES) {
    ...
    const hits = haystack.match(new RegExp(cat.match.source, 'gi'))?.length ?? 0;
    if (hits === 0) continue;
...
/**
 * Group items by their derived category. Seeded with all registry ids so the
 * returned map iterates in declared order even when buckets are empty.
 */
...
export function groupAgentsByCategory(agents: readonly Agent[]): Map<AgentCategoryId, Agent[]> {
...
/**
 * Direct `type → category` map for known MCPs (editorial control). Unknown
 * types fall through to the regex haystack. Lowercase keys.
 */
const MCP_TYPE_TO_CATEGORY: Readonly<Record<string, AgentCategoryId>> = {
  slack: 'tools-communication',
  gmail: 'tools-communication',
  google: 'tools-communication',
  microsoft: 'tools-communication',
...
export function groupMcpsByCategory(mcps: readonly McpServer[]): Map<AgentCategoryId, McpServer[]> {
  return groupByCategory(mcps, deriveMcpCategory);
}

/* ── Skills ────────────────────────────────────────────────────────── */

function buildSkillHaystack(skill: Skill): string {
  // Skip `content` — it's arbitrarily long markdown and would swamp the regex.
  return [skill.name ?? '', skill.slug ?? '', skill.description ?? ''].join(' ');
}

export function deriveSkillCategory(skill: Skill): AgentCategoryId {
  return deriveCategoryFromHaystack(buildSkillHaystack(skill));
}

export function groupSkillsByCategory(skills: readonly Skill[]): Map<AgentCategoryId, Skill[]> {
  return groupByCategory(skills, deriveSkillCategory);
}
