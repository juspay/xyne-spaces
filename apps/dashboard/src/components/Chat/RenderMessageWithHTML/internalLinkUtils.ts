 ...
 * guests. That invite host is deployment config and usually is not the
...
export const parseInternalXyneLink = (href: string): ParsedInternalXyneLink | null => {
  // Checked before the host allowlist: an invite link points at the external
  // call host, which is not one of our app hosts.
