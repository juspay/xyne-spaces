...
function fromSandpack(
  ...
  const message = error.message.trim() || i18next.t('artifactErrorOverlay.unknownError');
  const location =
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent): void => {
      ...
      const next: ArtifactRenderError = {
        message: message?.trim() || t('artifactErrorOverlay.unknownError'),
      ...
      <div className='flex w-full max-w-md flex-col gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-4'>
        <div className='flex items-start gap-2'>
          ...
          <div className='min-w-0 flex-1'>
            <p className='text-sm font-medium text-foreground'>
              {t('artifactErrorOverlay.hitErrorTitle')}
            </p>
          ...
          <button
            ...
            aria-label={t('artifactErrorOverlay.dismissAriaLabel')}
            title={t('artifactErrorOverlay.dismissTitle')}
        ...
        <div className='flex flex-wrap items-center gap-2'>
            ...
            >
              {requested ? (
                <>
                  ...
                  {t('artifactErrorOverlay.fixRequested')}
                </>
              ) : (
                <>
                  <Sparkles className='h-3.5 w-3.5' aria-hidden='true' />
                  {t('artifactErrorOverlay.fixWithAi')}
                </>
              )}
            </button>
          )}
          ...
          >
            ...
            {copied ? t('artifactErrorOverlay.copied') : t('artifactErrorOverlay.copyError')}
          </button>
          {refused && (
            <span className='text-xs text-muted-foreground'>
              {t('artifactErrorOverlay.waitForReply')}
            </span>
          )}
        </div>
      </div>
