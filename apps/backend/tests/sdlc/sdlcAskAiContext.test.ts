...
import {
  ...
  resolveSdlcAskAiArtifactKind,
  resolveSdlcAskAiSelectedArtifact,
} from '../../src/sdlc/sdlcAskAiContext';
...
it('identifies the selected SDLC artifact and gives its canonical update contract', () => {
  const context = buildSdlcAskAiContext({
    repo: { id: 'repo-1', name: 'Hyper Switch', url: 'https://example.test/repo' },
    channelId: 'channel-1',
    baselineDocuments: [],
    ...
    selectedArtifact: {
      canvasId: 'canvas-1',
      title: 'Take this forward',
      artifactKind: 'PRD',
    },
  });

  assert.match(context, /Selected SDLC artifact/);
  assert.match(context, /Artifact kind: PRD/);
  assert.match(context, /Canvas ID: canvas-1/);
  assert.match(context, /already an SDLC artifact/i);
  assert.match(context, /spaces-sdlc-mutate-artifact/);
  assert.match(context, /action update/i);
  assert.match(context, /canvasId.*canvas-1/i);
  assert.match(context, /never classify.*regular canvas/i);
});

it('recognizes canonical Wiki metadata as an SDLC artifact', () => {
  assert.equal(
    resolveSdlcAskAiArtifactKind({ surface: 'SDLC', documentKind: 'WIKI' }),
    'WIKI'
  );
});

it('binds selected Wiki metadata to the owning SDLC repository only', () => {
  const wiki = {
    id: 'wiki-canvas-1',
    title: 'Architecture',
    metadata: { surface: 'SDLC', repoId: 'repo-1', documentKind: 'WIKI' },
  };

  assert.deepEqual(resolveSdlcAskAiSelectedArtifact(wiki, 'repo-1'), {
    canvasId: 'wiki-canvas-1',
    title: 'Architecture',
    artifactKind: 'WIKI',
  });
  assert.equal(resolveSdlcAskAiSelectedArtifact(wiki, 'repo-2'), undefined);
  assert.equal(
    resolveSdlcAskAiSelectedArtifact(
      { ...wiki, metadata: { ...wiki.metadata, surface: 'CHAT' } },
      'repo-1'
    ),
    undefined
  );
