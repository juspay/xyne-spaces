...
jest.mock('@xyne/shared', () => ({ SDLC_BASELINE_COUNT: 7 }));
...
describe('SdlcWikiPipeline repository gates', () => {
  ...
  it('resumes a cancelled run with its durable history-window assignment', async () => {
    ...
    const tx = {
      $queryRaw: jest.fn().mockResolvedValue([{ id: 'repo-1' }]),
      sdlcEntityLink: {
        findFirst: jest.fn().mockResolvedValue({ targetId: 'execution-1' }),
    ...
    expect(tx.sdlcEntityLink.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ targetId: 'execution-1' }) })
    );
  });

  it('rejects retry when another Wiki run became active while waiting for the repo lock', async () => {
    const serialized = JSON.stringify(cancelledContext());
    const tx = {
      $queryRaw: jest.fn().mockResolvedValue([{ id: 'repo-1' }]),
      sdlcEntityLink: {
        findFirst: jest.fn().mockResolvedValue({ targetId: 'execution-old' }),
        findMany: jest.fn().mockResolvedValue([{ targetId: 'execution-new' }]),
      },
      workflowExecution: {
        findUnique: jest.fn().mockResolvedValue({
          id: 'execution-old',
        ...
        findFirst: jest.fn().mockResolvedValue({ id: 'execution-new' }),
        updateMany: jest.fn(),
      },
    ...
    await expect(
      service.retry({ userId: 'user-1', workspaceId: 'workspace-1' }, 'repo-1', 'execution-old')
    ).rejects.toMatchObject({ statusCode: 409 });

    expect(tx.workflowExecution.updateMany).not.toHaveBeenCalled();
  });
