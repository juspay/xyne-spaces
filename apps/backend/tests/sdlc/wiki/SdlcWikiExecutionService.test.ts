jest.mock('../../../src/config/env', () => ({
  config: { xyneClaw: { callbackUrl: 'https://claw.example', s2sKey: 'test' } },
}));
...
import { getS2SClawRunStatus, runS2SClawAgent } from '../../../src/services/clawAgentService';
...
describe('SdlcWikiExecutionService deterministic preparation', () => {
  ...
  it('normalizes and deduplicates bootstrap page paths before dispatch', async () => {
    const bootstrapSha = 'b'.repeat(40);
    const dispatched: WikiExecutionContext = {
      ...context(), phase: 'BOOTSTRAPPING', bootstrapRef: bootstrapSha,
      assignedChunk: {
        kind: 'BOOTSTRAP_SURVEY', conversationId: 'conversation-1', sessionId: 'session-1',
        commitShas: [bootstrapSha], nextIndex: 0,
      },
    };
    const updateMany = jest.fn().mockResolvedValue({ count: 1 });
    const tx = {
    ...
    const prisma = {
      workflowExecution: { findUnique: jest.fn().mockResolvedValue({
        id: 'execution-1', workflowId: 'workflow-1', workflow: {}, workspaceId: 'workspace-1',
        status: 'RUNNING', createdBy: 'user-1', context: JSON.stringify(dispatched),
        output: JSON.stringify({ version: 1, outcomes: [] }),
      }) },
      $transaction: jest.fn(async (callback: (transaction: typeof tx) => unknown) => callback(tx)),
    };
    ...
    const service = new SdlcWikiExecutionService(prisma as never, queue);
    const page = {
      purpose: 'System entry point', concepts: ['System'], priority: 'HIGH', archetype: 'overview',
      sourceAreas: ['src'], relatedPages: [], tableCandidates: [], diagramCandidates: [],
    };

    await service.handleCallback('execution-1', 'BOOTSTRAP_SURVEY', {
      sessionId: 'session-1', status: 'completed',
      result: { repositorySummary: 'Repository', pages: [
        { ...page, path: 'overview' },
    ...
    expect(persisted.bootstrapPlan?.pages.map(candidate => candidate.path)).toEqual([
      'overview.md', 'subsystems/backend.md',
    ]);
  });
});
  ...
  it('requeues a v2 infrastructure failure without discarding its assigned window', async () => {
    ...
    const dispatched: WikiExecutionContext = {
      ...
      assignedChunk: {
        ...
        commitShas: [endpoint],
        nextIndex: 0,
        window: { beforeSha: SHA, afterSha: endpoint, activeCheckpointSha: null, completedCheckpointShas: [] },
      },
      counts: {
        total: 1,
        processed: 0,
    ...
    const prisma = {
      workflowExecution: {
        findUnique: jest
          .fn()
          .mockResolvedValueOnce({ id: 'execution-1', workflowId: 'workflow-1', workflow: {}, status: 'RUNNING', context: JSON.stringify(dispatched), output: JSON.stringify({ version: 2, outcomes: [] }) })
...
describe('SdlcWikiExecutionService staged bootstrap', () => {
  const plan = {
    ...
    editorialReports: [],
    pages: [{
      path: 'overview.md', purpose: 'Overview', concepts: ['System'], priority: 'HIGH' as const,
      archetype: 'overview' as const, sourceAreas: ['src'], relatedPages: [],
  ...
  function callbackHarness(value: WikiExecutionContext, durableOverride?: WikiExecutionContext) {
    ...
    const durable = value.assignedChunk?.kind === 'BOOTSTRAP_PAGE'
      ? {
          ...value,
          pendingCommit: {
            commitSha: value.bootstrapRef!,
            pages: [{
              path: value.bootstrapPlan?.correction?.path ?? value.bootstrapPlan?.pages[value.bootstrapPlan.nextPageIndex]?.path ?? 'overview.md',
      ...
      : value;
    const prisma = {
      workflowExecution: { findUnique: jest
        .fn()
        .mockResolvedValueOnce({
          id: 'execution-1', workflowId: 'workflow-1', workflow: {}, workspaceId: 'workspace-1',
          status: 'RUNNING', createdBy: 'user-1', context: JSON.stringify(value),
          output: JSON.stringify({ version: 1, outcomes: [] }),
        })
  ...
  it('moves one completed page to a separate editorial run in Standard quality', async () => {
    const value: WikiExecutionContext = {
      ...context(), phase: 'BOOTSTRAPPING', bootstrapRef: SHA, bootstrapPlan: plan,
      bootstrapStage: 'PAGE', assignedChunk: {
    ...
    await service.handleCallback('execution-1', 'BOOTSTRAP_PAGE', {
      sessionId: 'session-1', status: 'completed', result: { completed: true },
    });
  ...
  it('does not advance a bootstrap page from another session pending evidence', async () => {
    const secondPlan = {
      ...
      pages: [
        plan.pages[0],
        { ...plan.pages[0], path: 'backend.md', purpose: 'Backend' },
      ],
    };
    const value: WikiExecutionContext = {
      ...context(), phase: 'BOOTSTRAPPING', bootstrapRef: SHA, bootstrapPlan: secondPlan,
      sessionId: 'session-2', credentialSessionId: 'session-2',
      bootstrapStage: 'PAGE', assignedChunk: {
        kind: 'BOOTSTRAP_PAGE', conversationId: 'conversation-2', sessionId: 'session-2',
        commitShas: [SHA], nextIndex: 0,
      },
    };
    const durable: WikiExecutionContext = {
      ...
      pendingCommit: {
        commitSha: SHA,
        pages: [{
          path: 'backend.md', requestHash: 'old-request', writerSessionId: 'session-1',
          revision: {
            action: 'created', commitSha: SHA, canvasId: 'canvas-old',
            canvasVersionId: 'version-old', contentHash: 'content-old', sourcePaths: ['src/index.ts'],
          },
        }],
      },
    };
    ...
    await expect(service.handleCallback('execution-1', 'BOOTSTRAP_PAGE', {
      sessionId: 'session-2', status: 'completed', result: { completed: true },
    })).resolves.toBeUndefined();
  ...
  it('schedules a serialized correction when the editorial role finds an issue', async () => {
    const value: WikiExecutionContext = {
      ...context(), phase: 'BOOTSTRAPPING', bootstrapRef: SHA,
      bootstrapPlan: { ...plan, nextPageIndex: 1, pendingEditorialPath: 'overview.md' },
      bootstrapStage: 'EDITOR', assignedChunk: {
        kind: 'BOOTSTRAP_EDITOR', conversationId: 'conversation-1', sessionId: 'session-1',
        commitShas: [SHA], nextIndex: 0,
      },
    };
    const { service, updateMany } = callbackHarness(value);
    await service.handleCallback('execution-1', 'BOOTSTRAP_EDITOR', {
      sessionId: 'session-1', status: 'completed',
      result: { complete: false, missingTopics: [], issues: ['Missing failure behavior'], suggestions: [] },
    });
    const next = JSON.parse(updateMany.mock.calls[0][0].data.context) as WikiExecutionContext;
  ...
  it('does not loop when the second editorial review still reports an issue', async () => {
    ...
    const value: WikiExecutionContext = {
      ...
      bootstrapStage: 'EDITOR', assignedChunk: {
        kind: 'BOOTSTRAP_EDITOR', conversationId: 'conversation-2', sessionId: 'session-2',
        commitShas: [SHA], nextIndex: 0,
      },
    };
    const { service, updateMany } = callbackHarness(value);
    await service.handleCallback('execution-1', 'BOOTSTRAP_EDITOR', {
      sessionId: 'session-2', status: 'completed', result: priorReport,
    });
    const next = JSON.parse(updateMany.mock.calls[0][0].data.context) as WikiExecutionContext;
