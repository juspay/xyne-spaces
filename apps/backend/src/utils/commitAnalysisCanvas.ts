...
// Render one PR (title, URL, author mention, ticket link, env + migration
// changes) into `blocks`. Shared by the main "Pull Requests & Tickets" section
// and the "Hotfix PRs" section so both render identically.
...
// Builds the "🔥 Hotfix PRs" section: heading + a note about the delta range +
// one entry per hotfix PR. Returns an empty block list when there are no hotfix
// PRs so callers can drop the section entirely.
...
async function findExistingAnalysisCanvas(conversationId: string | undefined) {
...
export async function upsertCommitAnalysisCanvas(
  ...
  try {
    const now = new Date();
    const existing = await findExistingAnalysisCanvas(metadata.conversationId);
