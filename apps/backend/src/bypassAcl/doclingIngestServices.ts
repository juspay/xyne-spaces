 ...
 */
export function commitDoclingSplitAsServiceActor<T>(workspaceId: string, fn: () => Promise<T>): Promise<T> {
  return asService(
 ...
 */
export function routeDoclingIntakeAsServiceActor<T>(workspaceId: string, fn: () => Promise<T>): Promise<T> {
  return asService(
    ['DoclingAsyncFile'],
