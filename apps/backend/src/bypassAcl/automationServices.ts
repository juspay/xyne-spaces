 ...
 */
export function runScheduledAutomationJob<T>(workspaceId: string, fn: () => Promise<T>): Promise<T> {
  return asService(
    ['WorkflowExecution'],
 ...
 * runScheduledAutomationJob: a queue job has no caller, the executor's writes need an open scope.
 */
export function runAutomationJob<T>(workspaceId: string, fn: () => Promise<T>): Promise<T> {
  return asService(
    ['WorkflowExecution'],
 ...
 */
export function createAutomationExecutionForEvent<T>(workspaceId: string, fn: () => Promise<T>): Promise<T> {
  return asService(
    ['WorkflowExecution'],
 ...
 */
export function createAutomationExecutionForWebhook<T>(workspaceId: string, fn: () => Promise<T>): Promise<T> {
  return asService(
    ['WorkflowExecution'],
 ...
 */
export function sendApprovalNotificationAsUser<T>(
  fromUserId: string,
  workspaceId: string,
  fn: () => Promise<T>,
): Promise<T> {
 ...
 */
export function runDeskLabelBackfillJob<T>(workspaceId: string, fn: () => Promise<T>): Promise<T> {
  return asService(
