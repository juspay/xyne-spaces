...
/**
 * What a canvas sync is reacting to. A discriminated union rather than a flag
 * plus an optional level, so a grant cannot reach the canvas without saying
 * which level it is granting and silently land on VIEWER.
 */
type CanvasSyncAction =
  | { kind: 'grant'; access: GrantableEntityUserAccess }
  | { kind: 'revoke' };
...
export class RecordingSharingService {
  ...
  private async revoke(
    ...
    const { shares, activities } = await this.runTransaction(async tx => {
      ...
      for (const target of uniqueTargets) {
        ...
        await this.syncCanvasAccess(tx, recording, actor.workspaceId, target, { kind: 'revoke' });
  ...
  private async setAccess(
    ...
    await this.syncCanvasAccess(tx, recording, workspaceId, target, { kind: 'grant', access });
  ...
  private async syncCanvasAccess(
    ...
    for (const canvasId of canvasIds) {
      ...
      if (action.kind === 'grant') {
        ...
        const role = canvasRoleFor(action.access);
