...
export class CallShareService {
  ...
  /**
   * The single place call access levels are resolved. Authorization gates — the
   * REST controllers, the sharing command path and the Zero mutators — all read
   * their answer from here so the ranking rules live in exactly one place.
   *
   * Resolution collects every route the caller has in and keeps the strongest:
   *
   * - creator                     -> `owner`
  ...
  ): Promise<CallAccessLevel | null> {
    if (call.workspaceId !== workspaceId) return null;
    if (call.createdByUserId === userId) return 'owner';
  ...
  async hasAtLeast(
    ...
    const level = await this.getEffectiveAccess(call, userId, workspaceId, tx);
    return recordingAccessAtLeast(level, required);
  }
