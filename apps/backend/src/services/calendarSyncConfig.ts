 ...
 * a time (CALENDAR_SYNC_QUEUE_CONCURRENCY).
 *
 * Concurrency is safe ACROSS sources but not WITHIN one: two jobs for the same
 * source would race on the provider cursor (Google syncToken / Graph deltaLink),
 * and the loser would persist a stale cursor and silently drop the changes the
 * winner had already consumed. Before concurrency this was free — Bull's default
 * of 1 serialised everything — so the lock is what preserves that guarantee.
 *
 * TTL outlives the slowest realistic sync (paging + up to
 * MAX_CALENDAR_EVENTS_PER_SYNC upserts) so a healthy holder is never evicted
 * mid-run, while still bounding how long a crashed worker can wedge one source.
 */
export const CALENDAR_SOURCE_LOCK_TTL_SECONDS = 300;

/**
 * How long a job waits for a busy source before giving up and letting Bull
 * retry it with backoff. Long enough to absorb a short overlap (a coalesced
 * webhook landing on top of a manual sync), short enough not to occupy a
