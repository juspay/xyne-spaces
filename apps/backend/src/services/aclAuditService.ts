...
import { getContextOrNull, isSystemWorkspaceId } from '@/database/tenant/context';
...
export class ACLAuditService {
  ...
  async logEvent(
    ...
    try {
      ...
      // `runAsSystem` reaches here with SYSTEM_WORKSPACE_ID, which is truthy — deliberate, so
      // seed-acl's events are still recorded against the NOT NULL column. Not tenant rows.
      if (isSystemWorkspaceId(workspaceId)) {
        logger.warn(`ACL audit recorded against system sentinel: ${eventType} - ${description}`);
      }
