...
import { ChannelRole, ChannelAddUserPolicy, ChannelScopeType } from '@xyne/shared';
...
import { shareAgentConversationToChannelTx } from '@/bypassAcl/transactions/shareAgentConversationService';
...
export type SharedCitationInvocation = Prisma.InputJsonObject & {
  toolCallId: string;
...
export function destinationNoun(scopeType: string): string {
...
export async function shareAgentConversationToChannel(input: ShareInput): Promise<ShareResult> {
  ...
  const persisted = await shareAgentConversationToChannelTx(workspaceId, sourceConversationId, targetChannelId, userId, shareOperationId, tipMessageId, reShareConfirmed, channel, addAgentConfirmed, agentAppUser, agentSeenCutoffAt, agentSlug, agentName, visibleCount, trimmedNote, citationMetadata, markdown);
