...
export class ReleaseService {
	...
	async release(
		...
		// Best-effort timeline event — start of analysis. Wrapped so a failure
		// here never breaks the actual release flow (the event log is observability,
		...
		// 0-apps-matched early return AND the happy-path result. This is the
		// data the controller renders as "0/2 applications matched" with the
		// per-app file counts.
		...
		// Emit one SUBTICKET event per successfully-provisioned application so
		// the timeline shows "Backend API was prepared", "Dashboard was prepared", etc.
