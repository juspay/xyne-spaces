  ...
  try {
    ...
    for (const ticket of tickets) {
      ...
      if (!deployedCommitId || !newCommitId) {
        // This candidate has no analyzed range yet (e.g. a just-created release
        // ticket). Skip it and try the next active ticket on the branch rather
        // than abandoning the whole sync.
