...
export class WorkflowController {
  ...
  // ========== TEST ARTIFACTS METHODS (Preview Changes Tab) ==========

  /**
   * GET: List all test artifacts for a workflow execution.
   * Returns screenshots, reports, and logs from GCS.
   */
  getTestArtifacts = async (req: Request, res: Response): Promise<void> => {
    try {
      const { executionId } = req.params;
      ...
      if (cucumberReport) {
        try {
          ...
          logger.info(`[TEST-ARTIFACTS] Parsed test summary:`, testSummary);
        } catch (e) {
          logger.warn(`[TEST-ARTIFACTS] Failed to parse cucumber report for ${executionId}:`, e);
        }
      }
      ...
      logger.error('[TEST-ARTIFACTS] Error getting test artifacts:', error);
      res.status(500).json({
        error: 'Failed to get test artifacts',
        message: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  };
  ...
  /**
   * GET: Proxy an artifact image from GCS.
   * Query param: path=workflow-artifacts/{execId}/screenshots/test.png
   */
  getArtifactImage = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Validate the path starts with workflow-artifacts/ to prevent arbitrary GCS access
      if (!gcsPath.startsWith('workflow-artifacts/')) {
      ...
      logger.error(`[ARTIFACT-IMAGE] Error fetching image from GCS path=${req.query.path}:`, error);
      if (errorMsg.includes('does not exist') || errorMsg.includes('Not Found')) {
        res.status(404).json({ error: 'Image not found', details: errorMsg });
      } else {
        res.status(500).json({
  ...
  /**
   * GET: Download an artifact from GCS.
   * Query param: path=workflow-artifacts/{execId}/reports/report.json
   */
  downloadArtifact = async (req: Request, res: Response): Promise<void> => {
    try {
      const gcsPath = req.query.path as string;
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      logger.error(`[ARTIFACT-DOWNLOAD] Error fetching artifact from GCS path=${req.query.path}:`, error);
      if (errorMsg.includes('does not exist') || errorMsg.includes('Not Found')) {
        res.status(404).json({ error: 'Artifact not found', details: errorMsg });
      } else {
        res.status(500).json({
  ...
  /**
   * GET: View an artifact in browser (for iframe).
   * Query param: path=workflow-artifacts/{execId}/reports/report.html
   */
  viewArtifact = async (req: Request, res: Response): Promise<void> => {
    try {
      const gcsPath = req.query.path as string;
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      logger.error(`[ARTIFACT-VIEW] Error fetching artifact from GCS path=${req.query.path}:`, error);
      if (errorMsg.includes('does not exist') || errorMsg.includes('Not Found')) {
        res.status(404).json({ error: 'Artifact not found', details: errorMsg });
      } else {
        res.status(500).json({
          error: 'Failed to view artifact',
          message: errorMsg,
        });
      }
    }
