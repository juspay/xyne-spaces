...
async function postLoadingMessage(conversationId: string, userId: string): Promise<string> {
  ...
  // One analysis message per release conversation: reuse the existing one
  // (from the initial analysis / a prior hotfix sync) and update it in place
  // instead of posting a fresh message every run — otherwise each re-run piles
  // up another duplicate "Analysis Complete" card in the thread.
...
export class CommitAnalysisController {
  ...
  async analyzeCommits(params: CommitAnalysisParams): Promise<CommitAnalysisResponse> {
    ...
    try {
      ...
      const vcsClient = buildVcsClient(derivedVcsProvider);
