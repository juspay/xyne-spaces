...
async function decryptRunResult(result: unknown): Promise<unknown> {
  if (typeof result === 'string') {
    const [decrypted] = await getEncryptionProvider().decryptBatch([result]);
    return decrypted;
  }
  ...
  const decrypted = await getEncryptionProvider().decryptBatch(refs.map((ref) => ref.value));
...
export function wrapTransactionWithEncryption(
  ...
  if (!isEncryptionCapabilityEnabled('dbEncryption')) {
    return tx;
  }
