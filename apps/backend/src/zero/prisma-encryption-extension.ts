...
async function decryptConfiguredFields<T>(model: string, result: T): Promise<T> {
  ...
  const decryptedValues = await getEncryptionProvider().decryptBatch(refs.map((ref) => ref.value));
...
async function encryptRecordFields(
  ...
  const encryptedValues = await getEncryptionProvider().encryptBatch(
  ...
  query: {
    $allModels: {
      async $allOperations({ model, operation, args, query }) {
        ...
        // Encrypt writes
        if (
          isEncryptionCapabilityEnabled('dbEncryption') && ["create", "update", "upsert", "createMany", "updateMany"].includes(operation)
        ) {
        ...
        // Decrypt reads
        if (
          isEncryptionCapabilityEnabled('dbEncryption') && ["create", "createMany", "findMany", "findFirst", "findUnique", "findFirstOrThrow", "findUniqueOrThrow"].includes(operation)
        ) {
