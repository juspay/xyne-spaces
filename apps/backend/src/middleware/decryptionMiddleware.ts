...
import { decryptRequestBody, encryptResponseBody } from '@/services/internal/encryption-client';
...
export async function decryptRequestBodyMiddleware(
  ...
  try {
    ...
    req.body = await decryptRequestBody(req.body, sessionId);
...
export function encryptResponseBodyMiddleware(
  ...
  res.json = ((body?: unknown): Response => {
    ...
    void encryptResponseBody(body, sessionId)
      .then((encryptedBody) => originalJson(encryptedBody))
      .catch((error) => {
        logger.warn('[decryptionMiddleware] failed to encrypt response body; sending plaintext', {
          method: req.method,
          path: req.path,
          error: error instanceof Error ? error.message : String(error),
        });
