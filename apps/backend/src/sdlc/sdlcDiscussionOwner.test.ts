...
} from './sdlcDiscussionOwner.ts';

void test('resolves a Tech Doc selection to its PRD owner instead of trusting the requested owner', async () => {
...
void test('resolves a Pull Request through its Ticket and rejects cross-workspace surfaces', async () => {
  const lookup: SdlcDiscussionOwnerLookup = {
    getCanvas: async (id) =>
      id === 'prd-1'
        ? {
