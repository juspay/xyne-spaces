...
function artifactKindForCanvasType(
  ...
  if (canvasType === 'WIKI' || canvasType === 'PRD' || canvasType === 'TECH_DOC') {
    return canvasType;
  }
  return isBaselineCanvasType(canvasType) ? 'BASELINE' : null;
}
...
export class SdlcArtifactVersionStore {
  ...
  async listArtifacts(input: {
    ...
    const canvases = await this.prisma.canvas.findMany({
      where: {
        ...
        canvasType: { not: 'DEFAULT' },
        canvasStatus: { not: 'REFRESH_CANDIDATE' },
      },
    ...
    return canvases.flatMap(canvas => {
      ...
      const artifactKind = artifactKindForCanvasType(canvas.canvasType);
  ...
  private async resolveArtifact(input: {
    ...
    if (input.selector.type === 'WIKI_PAGE') {
      if (canvas.canvasType !== 'WIKI') throw new AppError('SDLC artifact not found', 404);
