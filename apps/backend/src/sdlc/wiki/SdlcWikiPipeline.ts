...
export interface SdlcWikiPipeline {
  ...
  retry(actor: SdlcWikiPipelineActor, repoId: string): Promise<SdlcWikiRunStatus>;
  cancel(actor: SdlcWikiPipelineActor, repoId: string): Promise<SdlcWikiRunStatus>;
  ...
  async retry(actor: SdlcWikiPipelineActor, repoId: string): Promise<SdlcWikiRunStatus> {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.workflow.update({
        where: { id: latest.execution.workflowId },
        data: { status: 'PENDING' },
      });
    });
    if (latest.context.sessionId && latest.execution.createdBy) {
      await cancelS2SClawRun(latest.context.sessionId, latest.execution.createdBy).catch(
        () => undefined
      );
    }
    await this.enqueueOrFail(latest.execution.id, repoId);
    const updated = await this.prisma.workflowExecution.findUniqueOrThrow({
      where: { id: latest.execution.id },
      select: { id: true, createdAt: true, updatedAt: true },
    });
    return this.statusFromExecution(updated.id, updated.createdAt, updated.updatedAt, context);
  }

  async cancel(actor: SdlcWikiPipelineActor, repoId: string): Promise<SdlcWikiRunStatus> {
    await this.requireRepository(actor, repoId, true);
    const latest = await this.latestRun(repoId);
    if (!latest || !ACTIVE_EXECUTION_STATUSES.includes(latest.execution.status as never)) {
      throw new AppError('No active Wiki run to cancel', 409);
    }
    const context: WikiExecutionContext = {
      ...latest.context,
      phase: 'CANCELLED',
    ...
    await sdlcAdmission.release(latest.context.admissionPermitId);
    const updated = await this.prisma.workflowExecution.findUniqueOrThrow({
      where: { id: latest.execution.id },
      select: { id: true, createdAt: true, updatedAt: true },
    });
