...
import { createRepositoryTx } from '@/bypassAcl/transactions/SdlcHubService';
import { createChannelTx } from '@/bypassAcl/transactions/SdlcHubService';
import { addChannelRepositoriesTx } from '@/bypassAcl/transactions/SdlcHubService';
import { createArtifactFromClawTx } from '@/bypassAcl/transactions/SdlcHubService';
import { updateArtifactFromClawTx } from '@/bypassAcl/transactions/SdlcHubService';
import { createTrackTx } from '@/bypassAcl/transactions/SdlcHubService';
...
export class SdlcHubService implements SdlcHub {
  ...
  async createArtifactFromClaw(
    ...
    const artifact = await commitAndSyncCanvasArtifact(
      () =>
        createArtifactFromClawTx(this, actor, input, content, channelId, folder, projectId, repo, hubKnowledge, artifactGenerationCommit, artifactSourceReferences, repoIds),
      syncToYSweet,
      actor.userId
    );
  ...
  async updateArtifactFromClaw(
    ...
    return commitAndSyncCanvasArtifact(
      () =>
        updateArtifactFromClawTx(this, existing, input, content, actor, repo, generationCommit, resolved),
      syncToYSweet,
      actor.userId
    );
  }
