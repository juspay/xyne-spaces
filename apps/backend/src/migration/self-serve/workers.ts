...
export class MigrationWorkers {
  ...
  private async refresh(job: MigrationJob): Promise<void> {
    // QUEUED is included: an interrupted refresh (pod restart / bumped Bull job) is re-enqueued by reconcile as QUEUED —
    // without this it would return here every pickup, so reconcile keeps re-enqueuing it → an endless pickup/re-enqueue
    // thrash that never runs the refresh. Accepting QUEUED lets the re-enqueued refresh actually resume.
