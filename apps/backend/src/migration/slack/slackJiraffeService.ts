...
import { ingestTicketTx } from '@/bypassAcl/transactions/slackJiraffeService';
...
async function ingestTicket(
  ...
  const createdTicket = await ingestTicketTx(db, boardProjectId, channelRepo, channelId, titleText, description, userId, conversation, boardId, resolvedUserGroupId, resolvedStageName, resolvedStatusV2, assignedToUserId, ticket, externalSource);
