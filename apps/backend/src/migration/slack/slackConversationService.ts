  ...
  try {
    ...
    if (input.xyneSpaceChannelId) {
      ...
      if (xyneChannel) {
        ...
        xyneSpaceChannelLink = `<${frontendUrl}/${workspaceId}/chat/dir/${input.xyneSpaceChannelId}|${channelName}>`;
    ...
    if (!batchFailed && wsConfig.notificationsEnabled && input.postChannelAnnouncement !== false) {
      ...
      const xyneSpacesLink = input.xyneSpaceChannelId && xyneSpaceWorkspaceId
        ? `<${frontendUrl}/${xyneSpaceWorkspaceId}/chat/dir/${input.xyneSpaceChannelId}|Xyne Spaces>`
        : 'Xyne Spaces';
...
export async function runMigrationDm(input: DmMigrationInput): Promise<MigrationResult> {
  ...
  try {
    ...
    const frontendUrl = (process.env.FRONTEND_URL || 'http://localhost:5173').replace(/\/+$/, '');
    const xyneSpaceChannelLink = `<${frontendUrl}/${xyneSpaceWorkspaceId}/chat/dir/${xyneSpaceChannelId}|${channelName}>`;
