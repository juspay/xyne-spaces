...
router.post('/register-client-key', async (req: Request, res: Response) => {
  ...
  if (!isEncryptionCapabilityEnabled('clientEncryption') && !isEncryptionCapabilityEnabled('apiTransitEncryption')) {
...
router.post('/workspaces/backfill-provision', async (req: Request, res: Response) => {
  ...
  if (!isEncryptionCapabilityEnabled('provisioning')) {
    res.status(409).json({ error: 'Encryption provisioning is disabled' });
    return;
  }
