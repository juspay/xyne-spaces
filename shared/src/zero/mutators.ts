...
export const mutators = defineMutators({
  ...
  collection: {
    createCollection: defineMutator(
      ...
      async ({ tx, ctx, args: { id, projectId, name, description, vespaDocId, permissionId, timestamp } }) => {
    ...
    createFolder: defineMutator(
      ...
      async ({ tx, ctx, args: { id, collectionId, parentId, name, path, vespaDocId, timestamp } }) => {
    ...
    deleteItem: defineMutator(
      ...
      async ({ tx, ctx, args: { id, collectionId, timestamp } }) => {
        ...
        const item = await tx.run(zql.collection_items.where('id', id).one());

        await tx.mutate.collection_items.update({ id, deletedAt: timestamp });

        // If deleting a folder, cascade soft-delete to all descendants via BFS
        if (item?.type === CollectionItemType.FOLDER) {
          const allItems = await tx.run(
            zql.collection_items.where('collectionId', collectionId).where('deletedAt', 'IS', null),
          );
