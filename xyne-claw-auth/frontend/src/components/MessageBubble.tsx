...
import { Loader2, Download, FileText, Check, X, Eye } from "lucide-react";
...
export interface BubbleMessage {
  ...
  role: "user" | "assistant";
  content: string;
  contextItems?: Array<{ type: "channel" | "ticket" | "canvas" | "call"; id: string; title: string }>;
...
export function MessageBubble({ message, userId }: { message: BubbleMessage; userId?: string }) {
  const { role, content, contextItems, status, images, files, reasoning, invocations, streaming, pendingActions, onApproveAction, onDeclineAction, footer } = message;
  ...
  const assistantHasAnything = hasReasoning || hasInvocations || hasText || hasFiles || hasPendingActions;
    ...
    <div className={`flex min-w-0 ${isUser ? "justify-end" : "justify-start"}`}>
      <div className="min-w-0 max-w-[75%]">
        ...
        {!hideBubble && (
          ...
          <div className={`rounded-lg px-4 py-2.5 text-sm ${bubbleClass}`}>
            {isUser ? (
              <>
                {images && images.length > 0 && (
                  <div className="mb-2 flex flex-wrap gap-1.5">
                    {images.map((src, i) => <img key={i} src={src} alt="" className="max-h-48 rounded-md" />)}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                {hasReasoning && <ReasoningBlock text={reasoning!} streaming={Boolean(streaming)} />}
                ...
                {hasInvocations && <InvocationBlocks invocations={invocations!} />}
                ...
                {hasText && (
                  <div className="prose prose-sm prose-invert max-w-none prose-p:my-1 prose-headings:my-2 prose-ul:my-1 prose-ol:my-1 prose-li:my-0 prose-pre:my-2 prose-table:my-2 prose-hr:my-2 prose-th:px-2 prose-th:py-1 prose-td:px-2 prose-td:py-1 prose-th:border prose-td:border prose-th:border-zinc-700 prose-td:border-zinc-700">
                    ...
                    {streaming && <span className="inline-block h-3 w-1.5 translate-y-[1px] animate-pulse bg-zinc-400" />}
                  </div>
                )}

                {/* Non-image file attachments — rendered as download cards. */}
                {hasFiles && <FileAttachments files={files!} userId={userId} />}
          ...
          {footer && <div className="mt-1 px-1 text-xs text-zinc-500">{footer}</div>}
          </>
        )}
      </div>
    </div>
...
function contextTypeLabel(type: "channel" | "ticket" | "canvas" | "call"): string {
...
function FileAttachments({ files, userId }: { files: BubbleFile[]; userId?: string }) {
  return (
...
function FileAttachmentCard({ file: f, userId }: { file: BubbleFile; userId?: string }) {
  ...
  const isPptx = userId && !isStreaming && (f.mimeType === PPTX_MIME || f.name.toLowerCase().endsWith(".pptx"));
  ...
  const openPreview = async () => {
    ...
    try {
      ...
      if (!data) setErr("Preview not available — slide JSON wasn't saved for this file.");
      else setViewer(data);
    } catch (e) {
  ...
  return (
    <>
      ...
      {viewer && <PptViewer config={viewer} fileName={f.name} onClose={() => setViewer(null)} />}
    </>
  );
}
...
function ReasoningBlock({ text, streaming }: { text: string; streaming: boolean }) {
  ...
  return (
    <div className="rounded-md border border-zinc-700 bg-zinc-900/50">
      <button onClick={() => setExpanded(!expanded)} className="flex w-full items-center gap-2 px-2 py-1.5 text-left text-[11px] text-zinc-400 transition hover:bg-zinc-900">
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`shrink-0 transition ${expanded ? "rotate-90" : ""}`}>
          <polyline points="9 18 15 12 9 6" />
        </svg>
        <span>🧠 {streaming ? "Thinking…" : "Thought"}</span>
        <span className="ml-auto font-mono text-zinc-600">{text.length} chars</span>
      </button>
      {expanded && (
        <pre className="max-h-64 overflow-auto whitespace-pre-wrap break-words border-t border-zinc-800 px-3 py-2 font-mono text-[11px] text-zinc-400">{text}</pre>
      )}
    </div>
  );
}
...
function InvocationItem({ invocation, children }: { invocation: ToolInvocation; children?: ToolInvocation[] }) {
  const [expanded, setExpanded] = useState(false);
  ...
  return (
    <div className={`rounded-md border ${invocation.isError ? "border-red-900/50 bg-red-950/20" : isRunning ? "border-blue-900/50 bg-blue-950/10" : isSubagent ? "border-purple-900/50 bg-purple-950/10" : "border-zinc-700 bg-zinc-900/50"}`}>
      <button onClick={() => setExpanded(!expanded)} className="flex w-full items-start gap-2 px-2 py-1.5 text-left">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 text-xs">
            {(isRunning || runningChildren > 0) && <Loader2 size={10} className="animate-spin text-blue-400" />}
            ...
            <span className="text-zinc-200">{humanizeToolName(invocation.toolName)}</span>
      ...
      {expanded && (
        <div className="space-y-2 border-t border-zinc-800 px-2 py-2 text-[11px]">
          <div>
            ...
            <pre className="max-h-48 overflow-auto whitespace-pre-wrap break-all rounded bg-zinc-950 p-2 font-mono text-zinc-300">{argsFull}</pre>
          </div>
          <div>
          ...
          {children && children.length > 0 && (
            <div>
              <div className="mb-0.5 text-zinc-500">Inner tool calls ({children.length})</div>
...
function PendingActionItem({
  ...
  const [state, setState] = useState<"idle" | "running" | "approved" | "declined" | "error">("idle");
  ...
  const argsPreview = (() => {
    ...
    } catch { return String(action.params); }
  })();
  ...
  return (
    <div className="rounded-md border border-yellow-800/50 bg-yellow-950/10 p-3">
      <div className="mb-2 flex items-center gap-2 text-xs">
        ...
        <span className="rounded bg-purple-950 px-1.5 py-0.5 text-[10px] text-purple-300">{action.serverType}</span>
        <span className="text-zinc-100 font-medium">{humanizeToolName(action.tool)}</span>
      </div>
      <pre className="mb-2 max-h-40 overflow-auto whitespace-pre-wrap break-all rounded bg-zinc-950 p-2 font-mono text-[11px] text-zinc-400">{argsPreview}</pre>
      {error && <div className="mb-2 rounded border border-red-900/50 bg-red-950/30 px-2 py-1 text-[11px] text-red-300">{error}</div>}
