...
import { slackSurfaceAdapter } from "./adapter.js";
...
describe("slack surface adapter signature verification", () => {
  ...
  it("accepts a valid Slack v0 signature", () => {
    ...
    expect(
      slackSurfaceAdapter.verifySignature(body, signed(body, "right-secret", timestamp), "right-secret"),
    ).toBe(true);
  });
  ...
  it("rejects a signature made with another install's secret", () => {
    ...
    const timestamp = Math.floor(Date.now() / 1000);
    expect(
      slackSurfaceAdapter.verifySignature(body, signed(body, "org-a-secret", timestamp), "org-b-secret"),
    ).toBe(false);
  });
});
...
describe("slack surface adapter inbound parsing", () => {
  ...
  it.each([
    ...
    expect(
      slackSurfaceAdapter.parseInbound({
        type: "event_callback",
        team_id: "T123",
        event_id: "EvIgnored",
        event,
