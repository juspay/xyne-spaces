...
function renderReadme(agent: ProjectBundleInput["agent"], subagentCount: number, skillCount: number, sessionId: string): string {
  return [
    ...
    "",
    "The remaining subagents (`spaces`, `bitbucket`, `grafana`) need MCP servers you'd set up yourself. `.mcp.json` has **commented-out templates** under `_disabled` — copy the ones you want into `mcpServers` and fill in the env values:",
    "",
    ...
    "",
    "`xyne-spaces` doesn't have a public MCP package yet — that subagent runs as persona-only unless you deploy the xyne-claw MCP adapter yourself.",
    "",
