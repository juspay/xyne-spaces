...
<header>DPIP report</header>`;
  ...
  it('uses UTC report date in attachment filename', () => {
    ...
    assert.equal(
      dpipReportFileName(now),
      'dpip-daily-report-v2-2026-07-30.html',
    );
    assert.equal(
      dpipOverviewFileName(now),
      'dpip-overview-v2-2026-07-30.html',
    );
  });
  ...
  it('uploads both HTML reports as attachments on one Xyne message', async () => {
    ...
    assert.match(
      fileNames[0] ?? '',
      /^dpip-daily-report-v2-\d{4}-\d{2}-\d{2}\.html$/,
    );
    assert.match(
      fileNames[1] ?? '',
      /^dpip-overview-v2-\d{4}-\d{2}-\d{2}\.html$/,
    );
