...
#
# Merge commits: a raw call that already exists on MERGE_HEAD (the incoming
# parent, typically origin/main) is not "new" on this branch. Count against
# max(HEAD, MERGE_HEAD) so merging main does not fail the guard for main's
# own raw SQL.
#
...
if [ -f "$MERGE_HEAD_FILE" ]; then
    MERGE_HEAD_SHA="$(tr -d '[:space:]' < "$MERGE_HEAD_FILE")"
fi
...
while IFS=$'\t' read -r status p1 p2; do
    ...
    if [ -n "$MERGE_HEAD_SHA" ]; then
        incoming_count=$(count_raw "${MERGE_HEAD_SHA}:" "$new_path")
        if [ "$incoming_count" -gt "$old_count" ]; then
