...
- [x] T12.1 Route baseline and Start Work through the existing Claw `ask-ai` agent; create PRDs and Tech Docs through interactive SDLC Ask AI; remove the local SDLC AI executors.
- [x] T12.2 Add an S2S-only SDLC execution profile for durable baseline, Wiki, and Start Work runs without creating a new agent.
...
- [x] T12.5 Create baseline canvases sequentially and create interactive PRD/Tech Doc canvases directly through the narrow artifact tool, without placeholder canvases.
- [x] T12.6 Persist Claw conversation/session identity, process S2S callbacks, and reconcile stale baseline, Wiki, and Start Work executions against Claw run state.
...
a deterministic structural audit.
...
- [ ] T28.9 Review reliability and data integrity against observed long-run failures: restart/orphan recovery,
      ...
      Current slice (2026-08-18): capacity waiting is bounded from Bull enqueue time and the generic pending
      reconciler uses the same configured budget; every dispatched Claw run persists a hard deadline and stable
      timeout code; Wiki/PRD/Tech Doc selection resolves canonical metadata and Canvas identity. Focused deadline,
      admission-worker, Wiki reconciliation, artifact-contract, and Ask AI context tests cover these transitions.
