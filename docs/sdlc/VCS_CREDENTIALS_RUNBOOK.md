...
SDLC_CAPACITY_WAIT_TIMEOUT_MS=86400000
SDLC_CAPACITY_RETRY_DELAY_MS=30000
SDLC_CLAW_RUN_TIMEOUT_MS=10800000
```
...
Capacity-blocked jobs retry after `SDLC_CAPACITY_RETRY_DELAY_MS` plus up to the smaller of five seconds or half
that delay as jitter, and fail after
`SDLC_CAPACITY_WAIT_TIMEOUT_MS`; PostgreSQL reconciliation uses the same wait limit for executions that have not
started. `SDLC_CLAW_RUN_TIMEOUT_MS` is a hard limit for each dispatched Claw run. Expiry best-effort cancels the
Claw session, records `CLAW_RUN_TIMED_OUT`, releases its admission permit, and leaves the durable execution
retryable. Keep these values aligned across every API/worker process that can initialize or consume the `sdlc`
queue.
