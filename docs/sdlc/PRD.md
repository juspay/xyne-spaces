...
and three per repository by default, with repository round-robin admission. Capacity-blocked jobs retry with a
bounded delay for a configurable wait budget (24 hours by default); the durable pending-execution reconciler uses
the same budget rather than failing queued work early. Each dispatched Claw run also has a persisted configurable
wall-clock deadline (three hours by default). Expiry best-effort cancels the Claw session, records a stable timeout
code, releases admission, and leaves truthful retryable execution state. Queue history initially uses count-based
retention (100 completed and 500 failed jobs); production-scale time-based retention remains a follow-up.
...
narrow SDLC artifact tool. The dashboard collects the title, optional direction, and parent PRD when required,
then starts a fresh Ask AI conversation and submits that request. There is no separate artifact workflow or
placeholder canvas. The interactive session receives repository identity and tool instructions so PRDs or
Tech Docs land in the correct repository folders. Generated content is never treated as approved baseline
memory automatically.

When the SDLC Assistant opens on a selected SDLC Canvas, its trusted context includes the canonical Canvas ID,
title, repository, and artifact kind. PRD and Tech Doc updates reuse that Canvas ID through the narrow artifact
tool instead of creating replacements or falling back to generic Canvas mutation. Wiki identity is read from its
canonical `documentKind: WIKI` metadata; baseline, PRD, and Tech Doc identity remains `artifactKind` metadata.
