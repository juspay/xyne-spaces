      ...
      try {
        ...
        try {
          ...
          if (mcpCallResult.content && mcpCallResult.content.length > 0) {
            // Get text from first content item if it exists. The content block is a
            // union (text | image | audio | resource) and only the text variant
            // carries `.text`, so narrow before reading it. Non-text blocks yield ''
            // here, matching the previous `?.text || ''` behaviour.
            const firstContent = mcpCallResult.content[0];
            extractedData = (firstContent?.type === 'text' ? firstContent.text : '') as TOutput;
          } else {
