...
} from "../../../src/tools/sandbox/sdlc-credential-bootstrap.js";
...
describe("SDLC sandbox credential bootstrap", () => {
  it("fails closed before network redemption when Node crypto is unavailable", async () => {
    ...
    await expect(
      installSdlcGitCredentialBootstrap(mockSession("sandbox-1", 1), binding),
  ...
  it("requests a fresh public-key-bound envelope when a cached sandbox is reused", async () => {
    ...
    await installSdlcGitCredentialBootstrap(
      mockSession("sandbox-1", 0, "public-key-1"),
      binding,
    );
    await installSdlcGitCredentialBootstrap(
      mockSession("sandbox-1", 0, "public-key-2"),
      binding,
    );
    ...
    expect(calls.map(({ body }) => body.sandboxId)).toEqual([
      "sandbox-1",
      "sandbox-1",
    ]);
    expect(calls.map(({ body }) => body.sandboxPublicKey)).toEqual([
      "public-key-1",
      "public-key-2",
    ]);
    expect(calls.every(({ body }) => body.agentSlug === "sdlc-agent")).toBe(
      true,
    );
  });
  ...
  it("keeps PAT identity discovery and commit attribution inside the sandbox script", async () => {
    ...
    const hookBase64 = script.match(
      /post-commit",Buffer\.from\("([A-Za-z0-9+/=]+)","base64"\)/,
    )?.[1];
  ...
  it("rewrites both commit author and committer to the sandbox-fetched PAT identity", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        buildSdlcPostCommitHook(identity),
        { mode: 0o700 },
      );
      ...
      execFileSync("git", ["-C", repo, "commit", "-m", "test"], {
        stdio: "ignore",
      });
