...
export function uiWidgetValidationError(value: unknown): string | null {
  ...
  if (candidate["type"] === "pr") {
    if (candidate["operation"] !== "upsert") return "pr widgets use operation 'upsert'";
    return hasValidPrPayload(payload) ? null : "pr payload is missing a valid provider, status or title";
  }
