...
const PLACEHOLDER_PREFIX = "__DESK_CHART_";
...
function extractAndRenderCharts(md: string): { md: string; charts: Map<string, string> } {
  ...
  const normalized = md
    .replace(/\*{1,2}```chart\n?([\s\S]*?)```\*{0,2}/g, "```chart\n$1\n```")
    .replace(/```chart\s+(\{[\s\S]*?\})\s*$/gm, "```chart\n$1\n```");
  ...
  const processed = normalized.replace(/^```chart\n([\s\S]*?)^```/gm, (_match, json: string) => {
    try {
      ...
      if (vt === "KPI" || vt === "KPI_COMPARE") {
        ...
        const key = `${PLACEHOLDER_PREFIX}${idx++}__`;
        charts.set(key, `<div style="margin:16px 0;">${card}</div>`);
        return `\n${key}\n`;
...
function reinsertCharts(html: string, charts: Map<string, string>): string {
  ...
  // Remove any orphaned DESK_CHART_N keys that weren't in the store (no chart data)
  result = result.replace(/<p>\s*DESK_CHART_\d+\s*<\/p>/g, "");
  result = result.replace(/DESK_CHART_\d+/g, "");
  return result;
}
...
<body>
  <div class="container">
    ...
    <footer>Generated by Xyne · ${new Date().toISOString()}</footer>
  </div>
...
export const createDeskReportTool: ToolDefinition = {
  ...
  async execute(params): Promise<string> {
    ...
    try {
      ...
      const fullHtml = buildDeskHtmlDocument(title, subtitle, bodyHtml);
