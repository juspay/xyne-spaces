...
export class TicketsACL extends BaseQueryACL<'tickets'> {
  ...
  // NOTE: 'tickets' is opted out of the define-query.ts workspace backstop (Slack-Connect),
  // so every branch must be fully self-scoping (no reliance on a root workspaceId re-clamp).
  // A ticket is visible iff its channel is visible to me (same-workspace rule OR active connect
  ...
  canSelect<TReturn>(query: Query<'tickets', Schema, TReturn>, args?: SelectArgs): Query<'tickets', Schema, TReturn> {
    ...
    // backstop for tenant isolation; now that 'tickets' opts out, it must scope itself.
